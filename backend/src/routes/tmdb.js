...
// Discover movies endpoint (proxy for TMDB discover API)
router.get('/discover', rateLimiters.tmdb, cacheMiddleware(600), async (req, res) => {
  try {
    if (!TMDB_API_KEY) {
    ...
    // Build TMDB discover API URL with all query params
    ...
    const tmdbUrl = `${TMDB_BASE_URL}/discover/${mediaType}?api_key=${TMDB_API_KEY}&page=${page}${queryString ? `&${queryString}` : ''}`;

    console.log('Fetching discover content from:', tmdbUrl);
    ...
    console.error('Error fetching discover content:', {
