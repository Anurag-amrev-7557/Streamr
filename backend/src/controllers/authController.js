...
// Octokit is an ES module, so we'll handle it dynamically when needed
let octokit = null;

// Initialize Octokit when needed
...
exports.googleAuth = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // Update existing user's profile picture if it's not set or if it's different
...
exports.githubAuth = async (req, res) => {
  try {
    ...
    const { data } = await octokitInstance.request('POST /login/oauth/access_token', {
    ...
    // Get user data
    const { data: userData } = await octokitInstance.request('GET /user', {
      headers: {
