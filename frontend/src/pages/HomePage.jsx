...
import { cancelRaf } from '../utils/throttledRaf';
    ...
    >
      <div className={`relative ${isMobile ? 'aspect-[2/3]' : 'aspect-[16/10]'} rounded-lg overflow-hidden transform-gpu transition-all duration-700 hover:scale-[1.015] hover:shadow-2xl hover:shadow-black/20 w-full active:scale-[0.995] active:shadow-lg`}>
        ...
        {isOpening && (
          ...
          {!isMobile && (
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent rounded-lg overflow-hidden opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-out"
            >
  ...
  useEffect(() => {
    try {
      ...
      return () => {
        try {
          if (mql) {
            if (mql.removeEventListener) mql.removeEventListener('change', applyClass);
            else if (mql.removeListener) mql.removeListener(applyClass);
          }
        } catch (_) {}
      };
    } catch (error) {
              ...
              <VisibleOnDemand placeholderHeight={220}>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'topRated')?.icon}</span>
