...
import { PageLoader, SectionLoader, CardLoader } from '../components/Loader';
import ContinueWatching from '../components/ContinueWatching';
import CastDetailsOverlay from '../components/CastDetailsOverlay';
...
// 🚀 ADVANCED: Zustand State Management Store
import { 
  useHomePageStore, 
  useSectionActions, 
  useUIActions,
...
import TopKoreanMoviesSection from '../components/TopKoreanMoviesSection';
import TopKoreanDramasSection from '../components/TopKoreanDramasSection';
...
const useDevicePerformanceHints = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hardwareConcurrency = typeof navigator !== 'undefined' && navigator.hardwareConcurrency ? navigator.hardwareConcurrency : undefined;
      const effectiveType = connection.effectiveType || '';
      const saveData = connection.saveData || false;

      // Consider low-end when deviceMemory <= 2GB, cores <= 2, or slow connection / save-data
      if ((typeof deviceMemory === 'number' && deviceMemory <= 2) ||
          (typeof hardwareConcurrency === 'number' && hardwareConcurrency <= 2) ||
          /(^2g$|^slow-2g$|^3g$)/i.test(effectiveType) || saveData) {
        isLowEndDevice = true;
      }
    } catch (_) {
      // keep default
...
const usePerformanceMonitoring = () => {
  ...
  const memIntervalMs = (import.meta && import.meta.env && import.meta.env.DEV) ? 10000 : 60000;
      ...
      const intervalId = memoryIntervalRef.current;
  ...
  const cacheIntervalMs = (import.meta && import.meta.env && import.meta.env.DEV) ? 30000 : 180000;
...
const MovieSectionSwiper = memo(({ title, movies, loading, onLoadMore, hasMore, currentPage, sectionKey, onMovieSelect, onMovieHover, onPrefetch, isMobile, isLowPower }) => {
  ...
  const [renderCount, setRenderCount] = useState(() => {
    // If device is in low-power mode or prefers reduced motion, reduce initial renders
    const initial = isMobile ? (isLowPower ? 4 : 6) : (isLowPower ? 6 : 10);
    return Math.min(Array.isArray(movies) ? movies.length : 0, initial);
  });
  
  // Mark content as ready when movies are loaded and not in loading state
  useEffect(() => {
      ...
      <div className="mt-8 px-4">
        ...
        {isLowPower ? (
          ...
          <motion.div 
            ...
            animate={isContentReady ? { opacity: 1, x: 0 } : { opacity: 0, x: -20 }}
            transition={{ 
              duration: 0.5, 
          ...
          {movies.map((movie, index) => (
            <motion.div
              ...
              transition={prefersReducedMotion ? { duration: 0 } : { 
                ...
                ease: [0.25, 0.46, 0.45, 0.94],
                delay: Math.min(0.15 + (index * 0.04), 0.6) // Faster stagger, capped at 0.6s
              }}
              style={{
                transform: 'translateZ(0)', // Force GPU acceleration
                willChange: index < 8 ? 'transform, opacity' : 'auto', // Only first 8 cards on mobile
              }}
            >
              <MemoizedMovieCard 
                {...movie} 
  ...
  return (
    <div className="mt-8">
      {/* 🚀 OPTIMIZED: Disable animations on low-end devices or reduced motion */}
      ...
      >
        ...
        <div 
          style={{
            ...
            willChange: isContentReady ? 'auto' : 'opacity',
          }}
        >
          <Swiper
            modules={[Navigation, A11y, Mousewheel, Keyboard, FreeMode]}
        ...
        <div className={`${sectionKey}-swiper-prev !w-10 !h-10 !bg-white/5 hover:!bg-white/12 !rounded-full !border !border-white/10 !transition-all !duration-300 !ease-out -translate-x-16 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 !absolute !left-0 !-translate-y-1/2 !top-[50%] !m-0 !z-20 flex items-center justify-center hover:shadow-xl hover:shadow-black/20`}>
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
          </svg>
        </div>
        <div className={`${sectionKey}-swiper-next !w-10 !h-10 !bg-white/5 hover:!bg-white/12 !rounded-full !border !border-white/10 !transition-all !duration-300 !ease-out translate-x-16 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 !absolute !right-0 !-translate-y-1/2 !top-[50%] !m-0 !z-20 flex items-center justify-center hover:shadow-xl hover:shadow-black/20`}>
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const retryTimerRef = useRef(null);
    ...
    useEffect(() => {
      if (imageError && retry < retryCount && src) {
        // schedule RAF and then a timeout; keep refs separate so we don't overwrite
        retryRafRef.current = scheduleRaf(() => {
          retryTimerRef.current = setTimeout(() => {
            setImageError(false);
            setRetry((r) => r + 1);
            // clear timer ref after invocation
      ...
      return () => {
        ...
        if (retryTimerRef.current) {
          try { clearTimeout(retryTimerRef.current); } catch (_) {}
          retryTimerRef.current = null;
        }
      };
    ...
    useEffect(() => {
      return () => {
        ...
        if (retryTimeoutRef.current) {
          try { clearTimeout(retryTimeoutRef.current); } catch (_) {}
  ...
  return (
    ...
    >
      <div className={`relative ${isMobile ? 'aspect-[2/3]' : 'aspect-[16/10]'} rounded-lg overflow-hidden transform-gpu transition-all duration-700 hover:scale-[1.015] hover:shadow-2xl hover:shadow-black/20 w-full active:scale-[0.995] active:shadow-lg`}>
        ...
        <div className="hidden md:block absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent rounded-b-lg p-3 sm:p-4 pointer-events-none z-10">
          <h3 className="text-white font-semibold text-sm sm:text-base md:text-lg truncate drop-shadow-lg">
            {title}
          </h3>
        </div>
...
const LazyMovieDetailsOverlay = lazy(() => import('../components/MovieDetailsOverlay'));
...
const HomePage = () => {
  ...
  const { updateUI, setSelectedMovie: setZustandSelectedMovie } = useUIActions();
  
  // Note: Store is ready to use. To migrate a section:
  // 1. Replace useState with useSectionMovies('sectionName')
  // 2. Use setSectionMovies('sectionName', data) instead of setState
  // 3. Access via: const movies = useSectionMovies('trending')
  // Example: const trendingMovies = useSectionMovies('trending');
  ...
  // FIXED: Memoize categories to prevent unnecessary re-renders
  ...
  const memoizedFeaturedContent = useMemo(() => {
    ...
    return {
      ...featuredContent,
      isInWatchlist: isInWatchlist(featuredContent.id),
      isResumeAvailable: viewingProgress && featuredContent.id ? 
        (viewingProgress[`movie_${featuredContent.id}`] || viewingProgress[`tv_${featuredContent.id}`])?.progress > 0 : false
    };
  }, [featuredContent, isInWatchlist, viewingProgress]);
  ...
  // Inject reduced-GPU stylesheet once and toggle root class when device hints indicate low power
  useEffect(() => {
    try {
      const id = 'homepage-reduced-gpu-styles';
      ...
      const containerSelector = '.performance-scroll';
      const container = document.querySelector(containerSelector) || document.body;
      const className = 'homepage-reduced-gpu';

      const applyClass = () => {
        // Only apply reduced GPU for low-end devices, not for reduced motion
        if (isLowEndDevice) {
      ...
      // Removed prefers-reduced-motion listener to enable animations on all devices
  ...
  }, [isLowEndDevice]); // Removed reducedMotion dependency
  ...
  // FIXED: Initialize data fetching with proper memory management and infinite loop prevention
  useEffect(() => {
    let isMounted = true;
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (shouldSkipInitialLoading()) {
          ...
          // Still fetch any missing sections in background with timeout
    ...
    try {
      ...
      try {
        result = await deduplicateRequest(requestKey, async () => {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error('Request timeout')), 20000); // Increased to 20 second timeout
          });
  ...
  const fetchMoviesForCategory = useCallback(async (category, page = 1) => {
    ...
    return deduplicateRequest(cacheKey, async () => {
      ...
      try {
        ...
        const timeout = networkConditions.current.isSlow ? 25000 : 20000;
  ...
  }, [loadingStates, setLoadingState, getSetterFunction]); // FIXED: Removed fetchMoviesForCategory to prevent infinite loops
  ...
  }, [categories, CACHE_DURATION]); // FIXED: Removed dataCache dependency to prevent infinite loops
  ...
  }, [setActiveCategory, setLoadingState, updateCategoryState, setError]); // FIXED: Removed dependencies that cause infinite loops
  ...
  const fetchFeaturedContent = useCallback(async () => {
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Trending content request timeout')), 20000); // Increased to 20 seconds for better reliability
      });
      ...
      if (!allCandidates.length) {
        ...
        console.log('Trending promise status:', trendingPromise.status);
        console.log('Popular TV promise status:', popularTVPromise.status);
        console.log('Now playing promise status:', nowPlayingPromise.status);
        
        if (trendingPromise.status === 'rejected') {
          console.error('Trending promise rejected:', trendingPromise.reason);
        }
        if (popularTVPromise.status === 'rejected') {
          console.error('Popular TV promise rejected:', popularTVPromise.reason);
        }
        if (nowPlayingPromise.status === 'rejected') {
          console.error('Now playing promise rejected:', nowPlayingPromise.reason);
        }
                  ...
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'trending')?.icon}</span>
                    ...
                    isMobile={isMobile || false}
                    isLowPower={getReducedMotionOrLowPower()}
                  />
                </VisibleOnDemand>
            ...
            <div className="mt-6">
              <TopRankedSection
                ...
                items={(trendingMovies || []).slice(0, 10).map(m => ({
                  ...
                  poster_path: m.poster_path ? `https://image.tmdb.org/t/p/w342${m.poster_path}` : (m.backdrop_path ? `https://image.tmdb.org/t/p/w342${m.backdrop_path}` : ''),
                  tag: m.recently_added ? 'Recently added' : ''
                }))}
              />
            </div>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'popularTV')?.icon}</span>
              ...
              />
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'popular')?.icon}</span>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'topRatedTV')?.icon}</span>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'nowPlaying')?.icon}</span>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'upcoming')?.icon}</span>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'airingToday')?.icon}</span>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'awardWinning')?.icon}</span>
                ...
                <VisibleOnDemand placeholderHeight={320}>
                  <MovieSectionSwiper 
                    title={(
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{categories.find(c => c.id === 'latest')?.icon}</span>
              ...
              <VisibleOnDemand placeholderHeight={320}>
                <MovieSectionSwiper 
                  title={(() => {
                    ...
                    return (
                      <span className="inline-flex items-center gap-2">
                        <span className="flex-shrink-0 text-white">{c?.icon}</span>
                        <span>{c?.label || ''}</span>
                      </span>
                    );
                  })()}
