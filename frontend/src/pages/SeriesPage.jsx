...
const EnhancedEpisodeList = lazy(() => import('../components/EnhancedEpisodeList'));
import { motion, AnimatePresence } from 'framer-motion';
import { debounce } from 'lodash';
import { useWatchlist } from '../contexts/WatchlistContext';
const EnhancedSearchBar = lazy(() => import('../components/EnhancedSearchBar'));
import searchHistoryService from '../services/searchHistoryService';
import { formatRating, toNumericRating } from '../utils/ratingUtils';
...
const SeriesCard = ({ series, onSeriesClick, onShowEpisodes, index = 0 }) => {
  ...
  const handleCardClick = useCallback(async (e) => {
    ...
    try {
      setIsOpening(true);
      const maybePromise = onSeriesClick(series);
...
const SeriesPage = () => {
        ...
        const transformedSeries = seriesData.map(series => {
          ...
          if (isAlreadyTransformed) {
            ...
            return {
              ...
              rating: toNumericRating(series.vote_average),
  ...
  }, [location.search, genres, activeCategory, selectedGenre, selectedYear]);
  ...
  const handleGenreSelect = useCallback(async (genre) => {
    // Accept either an id or a genre object; normalize to object
    if (!genre) {
      setSelectedGenre(null);
  ...
  const filterSeries = useCallback((seriesToFilter) => {
    return seriesToFilter.filter(series => {
      ...
      const matchesGenre = !selectedGenre || 
        (series.genre_ids && Array.isArray(series.genre_ids) && series.genre_ids.includes(selectedGenre.id));
