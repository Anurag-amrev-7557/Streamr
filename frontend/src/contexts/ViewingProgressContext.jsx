...
export const ViewingProgressProvider = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedProgress) {
        ...
        // Convert to continue watching array with TV show grouping
        ...
        // Mobile-specific debugging for data loading
        if (typeof window !== 'undefined' && window.innerWidth < 768) {
          console.log('📱 Mobile data loading debugging:', {
            totalItems: continueWatchingArray.length,
            tvShows: continueWatchingArray.filter(item => item.type === 'tv').map(item => ({
              id: item.id,
              title: item.title,
              poster_path: item.poster_path,
              backdrop_path: item.backdrop_path,
              hasPosterPath: !!item.poster_path,
              hasBackdropPath: !!item.backdrop_path
            })),
            movies: continueWatchingArray.filter(item => item.type === 'movie').map(item => ({
              id: item.id,
              title: item.title,
              poster_path: item.poster_path,
              backdrop_path: item.backdrop_path,
        ...
        // Group TV shows by ID and keep only the most recent episode for each show
        const groupedItems = [];
  ...
  // Manually refresh data from localStorage (for testing)
