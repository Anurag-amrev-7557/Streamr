...
const ActiveUsers = ({ className = '' }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 429) {
            ...
            setError(`Rate limited - retry after ${retryAfter}s`);
    ...
    // Set up interval to refresh data every 60 seconds as fallback (temporarily increased to avoid rate limits)
    // TODO: Reduce back to 30 seconds after backend rate limiting is updated and deployed
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="w-4 sm:w-5 h-3 sm:h-4 bg-gradient-to-r from-white/10 to-white/5 rounded-lg"
          />
