...
export default function TopKoreanMoviesSection() {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    discoverMovies({ with_original_language: 'ko', page: 1, vote_count_gte: 10, vote_average_gte: 6 })
      .then(res => {
        if (!mounted) return;
        if (res && res.movies) {
            const mapped = res.movies.slice(0, 12).map(m => ({ id: m.id, title: m.title || m.name, poster_path: m.poster || m.poster_path }));
            setItems(mapped);
          }
        setLoading(false);
      })
  ...
  return (
    <section className="px-6" style={{ overflowY: 'hidden' }}>
      ...
      `}</style>
      <div className="mb-3">
        <h2 className="text-xl font-semibold bg-gradient-to-r from-white via-white/90 to-white/60 bg-clip-text text-transparent relative">Top Rated Korean Movies</h2>
      </div>
      {loading ? (
        isMobile ? (
