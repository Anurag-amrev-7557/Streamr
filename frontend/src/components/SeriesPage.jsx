...
const SeriesPage = () => {
  ...
  useEffect(() => {
    if (inView && hasMore && !loading && !isLoadingMore) {
      // Call loadMoreSeries directly to avoid function dependency
      loadMoreSeries();
    }
  }, [inView, hasMore, loading, isLoadingMore]); // Removed loadMoreSeries dependency
  ...
  useEffect(() => {
    if (inView && hasMoreSearchResults && !isSearching && searchQuery) {
      ...
      // Call searchSeries directly to avoid function dependency
      searchSeries(searchQuery, nextPage);
    }
  }, [inView, hasMoreSearchResults, isSearching, searchQuery, searchPage]); // Removed searchSeries dependency, added searchPage
