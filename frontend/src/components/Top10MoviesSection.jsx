...
export default function Top10MoviesSection({ title = 'Top 10 Movies in Netflix Today' }) {
  ...
  useEffect(() => {
    ...
    (async function load() {
      try {
        const resp = await fetch(`${getApiUrl()}/tmdb/trending?media_type=movie&time_window=day&page=1`);
        if (!mounted) return;
        if (resp.ok) {
          const data = await resp.json();
          const topItems = (data.results || []).slice(0, 10).map(item => {
            let pp = item.poster_path || item.poster || null;
            if (pp && pp.startsWith('/')) pp = getOptimizedImageUrl(pp, 'w342');
            return {
              id: item.id,
              title: item.title || item.name || item.original_title,
              poster_path: pp,
              tag: item.vote_average >= 8.0 ? 'Trending' : null
            };
          });
          setItems(topItems);
        }
      } catch (err) {
        console.error('Error loading trending movies (light):', err);
      } finally {
