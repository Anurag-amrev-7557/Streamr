...
const StreamingPlayer = ({ 
  ...
  const startStallDetection = useCallback(() => {
    ...
    stallCheckIntervalRef.current = setInterval(() => {
      ...
      if (!isHidden && !isOffline && isOpen && !isLoading && timeSinceUpdate > 120000) {
        attemptRecovery('stall');
      }
    }, 30000);
  }, [isOffline, isOpen, isLoading, lastProgressUpdate]);
  ...
  const reloadIframe = useCallback(() => {
    if (!iframeRef.current || !streamingUrl) return;
    const seconds = totalDuration && currentProgress ? (currentProgress / 100) * totalDuration : 0;
    const resumeUrl = buildResumeUrl(streamingUrl, seconds);
    try {
  ...
  }, [streamingUrl, totalDuration, currentProgress, buildResumeUrl]);

  const attemptRecovery = useCallback((reason = 'unknown') => {
    const now = Date.now();
    // Debounce recoveries: at most once every 45s
    if (now - lastRecoveryAtRef.current < 45000) return;
    if (reloadAttemptsRef.current >= 3) return;
    lastRecoveryAtRef.current = now;
    reloadAttemptsRef.current += 1;
    console.log(`🔁 Attempting recovery (${reason}), attempt #${reloadAttemptsRef.current}`);
    reloadIframe();
  }, [reloadIframe]);
  ...
  const handleOnline = useCallback(() => {
    ...
    setTimeout(() => attemptRecovery('online'), 1000);
  }, [attemptRecovery]);
  ...
  const handleVisibilityChange = useCallback(() => {
    ...
    if (document.hidden) {
      ...
      // Resume lightweight tracking when visible again
      startPeriodicProgressCheck();
      setupUrlMonitoring();
      // If we were offline recently, try a gentle recovery
      if (!isOffline) {
        attemptRecovery('visible');
      }
    }
  }, [attemptRecovery, isOffline, setupUrlMonitoring, startPeriodicProgressCheck]);
  ...
  }, [isOpen, stopProgressTracking, handleOnline, handleOffline, handleVisibilityChange, saveProgress, startStallDetection]);
