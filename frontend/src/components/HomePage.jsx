...
const swiperStyles = `
  ...
  /* Enhanced movie card overlay styling */
...
const usePerformanceMonitoring = () => {
  ...
  const memoryIntervalRef = React.useRef(null);
  ...
  return (
    <div className="mt-8">
      ...
      <div className="relative group">
        ...
        <div ref={prevElRef} className="!w-10 !h-10 !bg-white/5 hover:!bg-white/10 !rounded-full !border !border-white/10 !transition-all !duration-300 opacity-0 group-hover:opacity-100 !absolute !left-0 !-translate-y-1/2 !top-[50%] !m-0 !z-20 hover:!shadow-lg hover:!shadow-black/20 flex items-center justify-center">
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </div>
        <div ref={nextElRef} className="!w-10 !h-10 !bg-white/5 hover:!bg-white/10 !rounded-full !border !border-white/10 !transition-all !duration-300 opacity-0 group-hover:opacity-100 !absolute !right-0 !-translate-y-1/2 !top-[50%] !m-0 !z-20 hover:!shadow-lg hover:!shadow-black/20 flex items-center justify-center">
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
      <div className={`relative ${isMobile ? 'aspect-[2/3]' : 'aspect-[16/10]'} rounded-lg overflow-hidden transform-gpu transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-black/20 w-full active:scale-[0.98] active:shadow-lg`}>
        ...
        >
          ...
          {!isMobile && (
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 rounded-lg">
              <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 md:p-3 lg:p-5 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
...
const HomePage = () => {
  ...
  const fetchPrioritySection = async (sectionKey) => {
    ...
    try {
      ...
      if (error.message === 'Request timeout') {
        ...
        setTimeout(() => {
          if (import.meta.env.DEV) {
            console.log(`🔄 Retrying fetch for ${sectionKey}...`);
          }
  ...
  }, [activeCategory]); // Removed handleCategoryChange dependency to prevent infinite loops
  ...
  }, [pageStates, getFetchFunction, setTrendingMovies, setPopularMovies, setTopRatedMovies, setUpcomingMovies, setActionMovies, setComedyMovies, setDramaMovies, setHorrorMovies, setSciFiMovies, setDocumentaryMovies, setFamilyMovies, setAnimationMovies, setAwardWinningMovies, setLatestMovies, setPopularTVShows, setTopRatedTVShows, setAiringTodayTVShows, setNowPlayingMovies]);
