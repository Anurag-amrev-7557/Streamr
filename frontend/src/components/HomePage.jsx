...
const CategorySwiper = memo(({ categories, activeCategory, onCategoryClick, isMobile }) => {
  // Track swiper to destroy on unmount
...
const MovieSectionSwiper = memo(({ title, movies, loading, onLoadMore, hasMore, currentPage, sectionKey, onMovieSelect, onMovieHover, onPrefetch, isMobile }) => {
  // Swiper instance and navigation refs to prevent leaks and DOM re-queries
  ...
  return (
    <div className="mt-8">
      ...
      <div className="relative group">
        ...
        <div ref={prevElRef} className={`movie-swiper-prev-${sectionKey} !w-10 !h-10 !bg-white/5 hover:!bg-white/10 !rounded-full !border !border-white/10 !transition-all !duration-300 opacity-0 group-hover:opacity-100 !absolute !left-0 !-translate-y-1/2 !top-[50%] !m-0 !z-20 hover:!shadow-lg hover:!shadow-black/20 flex items-center justify-center`}>
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </div>
        <div ref={nextElRef} className={`movie-swiper-next-${sectionKey} !w-10 !h-10 !bg-white/5 hover:!bg-white/10 !rounded-full !border !border-white/10 !transition-all !duration-300 opacity-0 group-hover:opacity-100 !absolute !right-0 !-translate-y-1/2 !top-[50%] !m-0 !z-20 hover:!shadow-lg hover:!shadow-black/20 flex items-center justify-center`}>
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const ProgressiveImage = memo(
  ({
    ...
    const retryDelayTimeoutRef = useRef(null);
    ...
    useEffect(() => {
      if (imageError && retry < retryCount && src) {
        retryTimeoutRef.current = requestAnimationFrame(() => {
          retryDelayTimeoutRef.current = setTimeout(() => {
            setImageError(false);
            setRetry((r) => r + 1);
      ...
      return () => {
        ...
        if (retryDelayTimeoutRef.current) {
          clearTimeout(retryDelayTimeoutRef.current);
          retryDelayTimeoutRef.current = null;
        }
      };
...
const HeroSection = memo(({ featuredContent, trendingMovies, onMovieSelect, onGenreClick, onCastMemberClick }) => {
  ...
  // Trailer modal and portal removed
  ...
  useEffect(() => {
    ...
    function animate(currentTime) {
      // Pause when tab is hidden or user prefers reduced motion
      ...
      let didUpdate = false;
      setParallax(prev => {
        const nextX = lerp(prev.x, parallaxTarget.x, 0.08);
        const nextY = lerp(prev.y, parallaxTarget.y, 0.08);

        const snappedX = Math.abs(nextX - parallaxTarget.x) < 0.01 ? parallaxTarget.x : nextX;
        const snappedY = Math.abs(nextY - parallaxTarget.y) < 0.01 ? parallaxTarget.y : nextY;

        if (snappedX === prev.x && snappedY === prev.y) {
          return prev; // No state change; avoid unnecessary re-render
        }
        didUpdate = true;
        return { x: snappedX, y: snappedY };
      });

      // Only continue the animation loop if a visible state change occurred
      if (running && didUpdate) {
        parallaxAnimRef.current = requestAnimationFrame(animate);
      } else {
        parallaxAnimRef.current = null; // stop when stable
      }
    }

    // Start loop only if motion isn't reduced
    const prefersReduced = typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (!prefersReduced) {
      parallaxAnimRef.current = requestAnimationFrame(animate);
    }
  ...
  }, [featuredContent?.id, parallaxTarget.x, parallaxTarget.y]);

  // Removed toast timeouts and state
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute inset-0 bg-cover bg-center transition-opacity duration-1000 will-change-transform transform-gpu"
        style={{
              ...
              >
                 <img
                  ...
                  alt={featuredContent.title}
                   className="w-[200px] sm:w-[280px] md:w-[300px] lg:w-[340px] max-w-full h-auto object-contain transform transition-all duration-300 hover:scale-105 opacity-0 animate-fadeIn will-change-transform"
                  onError={e => {
...
const HomePage = () => {
  ...
  // FIXED: Move pageStates definition to prevent hoisting issues
  ...
  const CACHE_DURATION = 60 * 60 * 1000; // Default TTL: 1 hour in milliseconds (was 30 minutes)
  const CACHE_VERSION = '2.0'; // Bump to invalidate old schema automatically
  const STALE_MAX_AGE = 12 * 60 * 60 * 1000; // Allow stale usage up to 12 hours (SWR) (was 6 hours)
  ...
  const SECTION_TTLS = useMemo(() => ({
    ...
    upcoming: 120 * 60 * 1000,     // 2 hours (was 1 hour)
    popularTV: 90 * 60 * 1000,     // 1.5 hours (was 45 minutes)
    topRatedTV: 90 * 60 * 1000,    // 1.5 hours (was 45 minutes)
    airingToday: 30 * 60 * 1000    // 30 minutes (was 15 minutes)
  }), []);
  ...
  useEffect(() => {
    // Store refreshFromStorage in a ref to avoid dependency issues
    const refreshFromStorageRef = { current: refreshFromStorage };
    
    // Refresh viewing progress to ensure continue watching section is up to date
    refreshFromStorageRef.current();
  ...
  }, []); // Remove refreshFromStorage dependency to prevent infinite loops
  ...
  const PREFETCH_CONFIG = {
    ...
    networkAware: true, // Adjust based on network speed
  ...
  const fetchInitialMovies = async () => {
    ...
    try {
      ...
      // Execute the optimized loading strategy
  ...
  // 🚀 ENHANCED: Advanced cache cleanup with intelligent eviction and performance monitoring
  ...
  }, [loadingStates.initial, setLoadingState]); // Remove tracked function dependencies to prevent infinite loops
  ...
  useEffect(() => {
    // Memory optimization interval using tracked function
    const memoryInterval = trackedSetInterval(() => {
  ...
  }, [isMobile, trendingMovies.length, popularMovies.length, topRatedMovies.length, upcomingMovies.length, activeCategory]); // FIXED: Dependencies are correct
    ...
    try {
      ...
      try {
        result = await deduplicateRequest(requestKey, async () => {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error('Request timeout')), 6000); // Reduced to 6 second timeout
          });
  ...
  useEffect(() => {
    ...
    if (currentCategory && currentCategory !== 'all' && currentCategory !== prevCategory) {
      ...
      const handleCategoryChangeRef = { current: handleCategoryChange };
      handleCategoryChangeRef.current(currentCategory);
    }
  }, [activeCategory]); // Remove handleCategoryChange dependency to prevent infinite loops
...
// 🚀 ENHANCED: Advanced performance monitoring with memory tracking and service worker integration
const usePerformanceMonitoring = () => {
  ...
  useEffect(() => {
    ...
    if (performance.memory) {
      ...
      memoryIntervalRef.current = setInterval(updateMemoryMetrics, 5000);
    ...
    const cacheInterval = setInterval(updateCacheMetrics, 10000);
