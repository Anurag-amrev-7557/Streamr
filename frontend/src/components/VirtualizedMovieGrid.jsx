...
const VirtualizedMovieGrid = ({
        ...
        >
          ...
          <div className="relative aspect-[2/3]">
            <OptimizedImage
              ...
              // Prioritize above-the-fold rows only
              priority={rowIndex < 2}
              sizes={`${Math.max(120, Math.round(itemWidth))}px`}
            />
            
            {/* Hover Overlay - FIXED positioning */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 overflow-hidden">
  ...
  return (
    <div 
      className={`${className} ultra-smooth-scroll performance-scroll`} 
      style={{ 
    ...
    >
      <Grid
        ...
        overscanRowCount={(navigator.hardwareConcurrency || 8) <= 4 ? 1 : 3}
        overscanColumnCount={(navigator.hardwareConcurrency || 8) <= 4 ? 0 : 2}
        useIsScrolling={true}
        itemData={{
          movies,
          gridConfig,
