...
const ContinueWatchingCard = React.memo(({ item, onClick, isMobile, onRemove }) => {
  ...
  const getProgressPercentage = () => {
    ...
    console.log('🎬 ContinueWatchingCard progress for', item.title, ':', {
      progress,
      itemType: item.type,
      itemId: item.id,
    ...
    >
      <div className={`relative ${isMobileState ? 'aspect-[2/3]' : 'aspect-[16/10]'} rounded-lg overflow-hidden transform-gpu transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-black/20 w-full active:scale-[0.98] active:shadow-lg`}>
        ...
        >
          ...
          {/* Movie info overlay - ALWAYS VISIBLE on desktop for landscape cards */}
          <div className="hidden md:block absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent opacity-100 transition-all duration-500 rounded-lg">
