...
const ContinueWatchingCard = React.memo(({ item, onClick, isMobile, onRemove }) => {
  ...
  useEffect(() => {
    const checkScreenSize = () => {
      ...
      // Debug mobile state for TV shows
      if (item.type === 'tv') {
        console.log('📱 Mobile state check for TV show:', {
      ...
      <div className={`relative ${isMobileState ? 'aspect-[2/3]' : 'aspect-[16/10]'} rounded-lg overflow-hidden transform-gpu transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-black/20 w-full active:scale-[0.98] active:shadow-lg`}>
        ...
        {onRemove && (
          <motion.button
            ...
            className="absolute top-2 right-2 z-10 w-8 h-8 bg-black/70 hover:bg-black/90 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-95"
            style={{
              WebkitTapHighlightColor: 'transparent',
              WebkitTouchCallout: 'none',
              WebkitUserSelect: 'none',
...
const ContinueWatching = ({ onMovieSelect, isMobile }) => {
  ...
  // Debug logging
  console.log('🎬 ContinueWatching component - Items:', continueWatching.length);
