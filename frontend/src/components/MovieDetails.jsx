...
const MovieDetails = () => {
  ...
  useEffect(() => {
    if (movieDetails) {
      ...
      // Show modal using centralized scheduler
      // Cancel any previous mount RAF and schedule a new one
      if (mountRafRef.current) {
        try { cancelRaf(mountRafRef.current); } catch (_) {}
        mountRafRef.current = null;
      }
      mountRafRef.current = scheduleRaf(() => setIsVisible(true));
      if (!mountRafRef.current) setTimeout(() => setIsVisible(true), 50);
    }
    return () => {
                  ...
                  {movieDetails.production_companies.map((company) => (
                    <div key={company.id} className="flex flex-col items-center">
                      <div className="w-full aspect-[3/2] bg-white rounded-lg p-4 flex items-center justify-center">
                        {company.logo_path ? (
                          <img
                            src={getTmdbImageUrl(company.logo_path, 'w185')}
                            alt={company.name}
