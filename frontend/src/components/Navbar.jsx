...
const MovieImage = React.memo(({ 
  ...
  const handleError = useCallback(() => {
    if (retryCount < maxRetries) {
      ...
      handleError.currentTimeout = retryTimeout;
    } else {
      setError(true);
      setIsLoading(false);
  ...
  useEffect(() => {
    return () => {
      ...
      if (handleError.currentTimeout) {
        clearTimeout(handleError.currentTimeout);
        handleError.currentTimeout = null;
      }
    };
...
const Navbar = ({ onMovieSelect }) => {
  ...
  }, [setSearchResults, setIsSearching, setShowResults, setSelectedIndex]);
  ...
  }, [setShowResults, setSelectedIndex, setIsSearchFocused]));
  ...
  }, [debouncedSearch, setSearchQuery, setShowResults, setSelectedIndex, setSearchResults, setIsSearching]);
        ...
        <div className="hidden sm:flex items-center gap-4 md:gap-6 lg:gap-8 ml-2">
          <Link
            ...
            onMouseEnter={() => {
              ...
              import('../../src/components/MoviesPage');
            }}
          >
          ...
          <Link
            ...
            onMouseEnter={() => {
              ...
              import('../../src/components/SeriesPage');
            }}
          >
            <span className="relative z-10 flex items-center gap-1.5">
          ...
          <Link
            ...
            onMouseEnter={() => {
              ...
              import('../../src/components/CommunityPage');
            }}
          >
            <span className="relative z-10 flex items-center gap-1.5">
                    ...
                    <div className="divide-y divide-white/10" role="listbox">
                      {[...Array(3)].map((_, i) => (
                        <motion.div 
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: 8 }}
                          transition={{ delay: i * 0.02, duration: 0.15, type: 'spring', stiffness: 150, damping: 20 }}
                        >
                      ...
                      <div className="divide-y divide-white/10" role="listbox">
                          {searchResults.map((movie, index) => (
                            ...
                            initial={{ opacity: 0, y: 16 }}
                              animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, y: 8 }}
                            transition={{ delay: index * 0.02, duration: 0.15, type: 'spring', stiffness: 150, damping: 20 }}
        ...
        {user ? (
          <div className="relative">
            <motion.button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-medium text-sm transition-all duration-200 ${
                isMenuOpen ? 'bg-white/20 ring-2 ring-white/30' : 'bg-white/10 hover:bg-white/15'
              }`}
            ...
            <AnimatePresence>
              {isMenuOpen && (
                <motion.div
                  ...
                  className="absolute right-0 mt-6 w-56 max-w-[80vw] bg-[#1a1d21]/95 backdrop-blur-md rounded-lg shadow-2xl border border-white/10 overflow-hidden z-[9999] md:hidden"
                ...
                >
                  ...
                  <div className="py-0.5">
                    <Link
                      to="/profile"
                      className="flex items-center gap-2.5 px-2.5 py-2 text-white/80 hover:text-white hover:bg-white/5 transition-all duration-200 group"
                      onClick={() => setIsMenuOpen(false)}
                    >
                      <div className="w-7 h-7 rounded-md bg-white/5 flex items-center justify-center group-hover:bg-white/10 transition-colors duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 transform transition-transform duration-200 group-hover:scale-110" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                          <circle cx="12" cy="7" r="4" />
                        </svg>
                      </div>
                      <span className="flex-1 font-medium text-lg">Profile</span>
                    ...
                    >
                      ...
                      <span className="flex-1 font-medium text-lg">My List</span>
                      <svg className="w-5 h-5 text-white/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                      </svg>
                    </Link>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2.5 px-2.5 py-2 text-white/80 hover:text-white hover:bg-white/5 transition-all duration-200 w-full text-left group"
                    >
                      <div className="w-7 h-7 rounded-md bg-white/5 flex items-center justify-center group-hover:bg-white/10 transition-colors duration-200">
                        <svg xmlns="http://www/w3.org/2000/svg" className="h-5 w-5 transform transition-transform duration-200 group-hover:scale-110" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" />
                      ...
                      <span className="flex-1 font-medium text-lg">Logout</span>
                    </button>
                  </div>
                </motion.div>
              )}
