...
import { getAvailableStreamingServices } from '../services/streamingService';
...
const StreamingServiceToggler = ({ 
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Auto-select first service if no current service is set
      if (!currentService && services.length > 0) {
        // Call onServiceChange directly instead of handleServiceSelect to avoid circular dependency
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      <button
        ...
        disabled={isLoading}
        className={`flex items-center gap-2 px-3 py-1.5 bg-black/60 hover:bg-black/80 backdrop-blur-sm rounded-lg text-white text-sm font-medium transition-all duration-200 border border-white/20 hover:border-white/40 ${
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 bg-black/90 backdrop-blur-md border border-white/20 rounded-lg shadow-xl z-50 overflow-hidden min-w-32 max-h-60 overflow-y-auto"
          >
            <div className="py-1">
