...
// Lazy-load heavy children to avoid parsing/executing until needed
const StreamingPlayer = React.lazy(() => import('./StreamingPlayer'));
const TVEpisodeSelector = React.lazy(() => import('./TVEpisodeSelector'));
const EnhancedSimilarContent = React.lazy(() => import('./EnhancedSimilarContent'));
...
const EnhancedLoadMoreButton = React.lazy(() => import('./enhanced/EnhancedLoadMoreButton'));
...
const CustomDropdown = React.memo(({ 
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute top-full left-0 right-0 mt-1 bg-[#1a1a1a]/95 ${supportsBackdropFilter() && isHighPerformanceDevice() ? 'backdrop-blur-md' : ''} border border-white/10 rounded-lg shadow-xl z-50 overflow-hidden`}
          >
  ...
  // Worker wrapper to offload heavy share image generation from main thread
  const createShareCardBlobWithWorker = async (details, cfg) => {
    // Try to create a worker if supported
    if (typeof window !== 'undefined' && 'Worker' in window) {
      try {
        // Lazy-create worker instance; reuse via closure variable
        if (!createShareCardBlobWithWorker._worker) {
          // Use Vite-compatible worker URL
          createShareCardBlobWithWorker._worker = new Worker(new URL('../workers/shareWorker.js', import.meta.url));
          createShareCardBlobWithWorker._promises = new Map();
          createShareCardBlobWithWorker._worker.onmessage = (ev) => {
            const { id, blob, error } = ev.data || {};
            const p = createShareCardBlobWithWorker._promises.get(id);
            if (!p) return;
            if (blob) p.resolve(blob);
            else p.reject(error || 'Worker error');
            createShareCardBlobWithWorker._promises.delete(id);
          };
        }

        const id = `share-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
        const worker = createShareCardBlobWithWorker._worker;

        return await new Promise((resolve, reject) => {
          createShareCardBlobWithWorker._promises.set(id, { resolve, reject });
          try {
            worker.postMessage({ id, details, config: cfg });
          } catch (err) {
            createShareCardBlobWithWorker._promises.delete(id);
            reject(err);
          }
          // Safety timeout
          setTimeout(() => {
            if (createShareCardBlobWithWorker._promises.has(id)) {
              createShareCardBlobWithWorker._promises.delete(id);
...
// RealTimeUpdateManager is provided by singleton service: import at top
...
// 🚀 ULTRA-FAST TRAILER LOADING OPTIMIZATIONS:
// 1. Eager YouTube script injection for instant availability
// 2. Parallel trailer data fetching when movie details load
// 3. Minimal UI preload state for instant visual feedback
// 4. Optimized YouTube player options for faster rendering

// Flag to track YouTube script loading
let youtubeScriptLoaded = false;
let youtubeScriptPromise = null;

// Preload YouTube player component and script for faster trailer loading
const preloadYouTubePlayer = () => {
  // Only load once
  if (youtubeScriptLoaded || youtubeScriptPromise) {
    return youtubeScriptPromise;
  }

  youtubeScriptPromise = Promise.all([
    // Import the react-youtube component
    import('react-youtube').catch(() => null),
...
// Preload on module initialization (before component mount)
if (typeof window !== 'undefined') {
  // Use requestIdleCallback if available for non-blocking preload
  if ('requestIdleCallback' in window) {
    requestIdleCallback(() => preloadYouTubePlayer(), { timeout: 2000 });
  } else {
    // Fallback to setTimeout
    setTimeout(() => preloadYouTubePlayer(), 100);
  }
}
        ...
        {similar.poster_path && !imageError ? (
          <>
            ...
            <img 
              ...
              alt={displayTitle} 
              className={`w-full h-full object-cover transition-all duration-300 ${isHighPerformanceDevice() ? 'group-hover:scale-105 will-change-transform transform-gpu' : ''} ${
                imageLoaded ? 'opacity-100' : 'opacity-0'
              }`}
              style={imageStyle}
              loading="lazy"
...
// Virtualized Cast list
const CastListVirtual = ({ items = [], onClick, width = '100%', itemHeight = 92 }) => {
  ...
  return (
    <VirtualList
      height={Math.min(items.length * itemHeight, 400)}
...
// Virtualized Similar movies list (horizontal)
const SimilarListVirtual = ({ items = [], onClick, itemWidth = 160, height = 280 }) => {
  if (!items || items.length === 0) return null;
  const Row = ({ index, style }) => {
    const movie = items[index];
    return (
      <div style={{ ...style, width: itemWidth, display: 'inline-block', padding: 8 }}>
        <SimilarMovieCard similar={movie} onClick={onClick} />
      </div>
    );
  };
  ...
  return (
    <div style={{ overflowX: 'auto', whiteSpace: 'nowrap' }}>
      <div style={{ height, display: 'inline-block' }}>
        {items.map((it, idx) => (
          <div key={it.id || idx} style={{ width: itemWidth, display: 'inline-block', verticalAlign: 'top' }}>
            <SimilarMovieCard similar={it} onClick={onClick} />
          </div>
        ))}
      </div>
    </div>
  ...
  const useOptimizedIsMobile = () => {
    ...
    React.useEffect(() => {
      ...
      function handleResize() {
        // Use shared scheduleRaf for better performance
        if (rafRef.current) {
          try { cancelRaf(rafRef.current); } catch (_) {}
        }
      ...
      // Also throttle scrollY updates for overlay to avoid frequent re-renders
      const onScroll = () => {
        if (rafRef.current) {
          try { cancelRaf(rafRef.current); } catch (_) {}
        }
...
const MovieDetailsOverlay = ({ movie, onClose, onMovieSelect, onGenreClick }) => {
  ...
  // 🚀 ULTRA-FAST TRAILER: Track YouTube player readiness for instant modal display
  const [youtubePlayerReady, setYoutubePlayerReady] = useState(youtubeScriptLoaded);
  ...
  // Helper to update individual loading state
  const setLoadingState = useCallback((key, value) => {
    setLoadingStates(prev => {
      if (prev[key] === value) return prev; // No update if same value
      return { ...prev, [key]: value };
    });
  }, []);

  // Backward compatibility aliases for existing code
  const isCastLoading = loadingStates.cast;
  const isSimilarLoading = loadingStates.similar;
  const isTrailerLoading = loadingStates.trailer;
  const isSimilarLoadingMore = loadingStates.similarMore;
  const setIsCastLoading = (v) => setLoadingState('cast', v);
  const setIsSimilarLoading = (v) => setLoadingState('similar', v);
  const setIsTrailerLoading = (v) => setLoadingState('trailer', v);
  const setIsSimilarLoadingMore = (v) => setLoadingState('similarMore', v);
  ...
  // 🚀 CONSOLIDATED: Share sheet state to prevent excessive re-renders
  ...
  // Helper to update individual share state
  const updateShareState = useCallback((updates) => {
    setShareState(prev => {
  ...
  const { showShareSheet, shareImageUrl, isGeneratingShare, isShareEditing, sharePanelExpanded, showShareEditor, isImageReady } = shareState;
  const setShowShareSheet = (v) => updateShareState({ showShareSheet: v });
  const setShareImageUrl = (v) => updateShareState({ shareImageUrl: v });
  const setIsGeneratingShare = (v) => updateShareState({ isGeneratingShare: v });
  const setIsShareEditing = (v) => updateShareState({ isShareEditing: v });
  const setSharePanelExpanded = (v) => updateShareState({ sharePanelExpanded: v });
  const setShowShareEditor = (v) => updateShareState({ showShareEditor: v });
  const setIsImageReady = (v) => updateShareState({ isImageReady: v });
  ...
  useEffect(() => {
    return () => {
      ...
      if (animationFrameRef.current) {
        try { cancelRaf(animationFrameRef.current); } catch (e) {}
  ...
  // 🚀 ULTRA-FAST TRAILER: Preload YouTube player on mount for instant modal display
  useEffect(() => {
    if (!isMountedRef.current) return;

    // Start preloading YouTube immediately
    preloadYouTubePlayer().then(() => {
      if (isMountedRef.current) {
        setYoutubePlayerReady(true);
      }
    }).catch(() => {
      // Silently fail - component will still work
    });
  }, []);
  ...
  useEffect(() => {
    ...
    // If the caller passed a prefilled movie object (contains poster/backdrop/title/overview)
    // use it immediately to avoid flashing the loading skeleton while the detailed fetch runs.
    const hasPrefill = !!(
      movie.poster_path || movie.poster || movie.backdrop || movie.backdrop_path || movie.overview || movie.title || movie.name
    );

    if (hasPrefill) {
      // Prefer preserving existing details when switching to the same id
      setMovieDetails(prev => (prev && prev.id === movie.id ? prev : movie));
      // Consider basic info already available
      try { setBasicLoading(false); } catch (e) { /* setBasicLoading may be declared later but will exist when effect runs */ }
    } else {
      setMovieDetails(null);
      try { setBasicLoading(true); } catch (e) { /* noop */ }
    }
  ...
  useEffect(() => {
    const optimizeImages = () => {
      ...
      // Only target images inside this overlay to avoid touching the whole document
      const root = contentRef.current || document.body;
      const images = root ? root.querySelectorAll('img') : document.querySelectorAll('img');
    ...
    const schedule = () => {
      if (typeof window !== 'undefined' && window.requestIdleCallback) {
        idleId = window.requestIdleCallback(() => {
          try { optimizeImages(); } catch (e) { /* swallow */ }
        }, { timeout: 1000 });
      } else {
    ...
    schedule();
    return () => {
      if (typeof window !== 'undefined' && window.cancelIdleCallback && idleId) {
  ...
  useEffect(() => {
    ...
    // Debounce increase to reduce frequent regenerations
    const delay = 500;
    shareRegenTimeoutRef.current = setTimeout(() => {
      // Use requestIdleCallback to do heavy work off the critical path
      let idleId = null;
      const run = () => {
        (async () => {
            try {
      ...
      if (typeof window !== 'undefined' && window.requestIdleCallback) {
        ...
        idleId = window.setTimeout(run, 50);
      }

      // Ensure we clear idle callback if component unmounts
      shareRegenTimeoutRef.current = idleId;
    }, delay);

    return () => {
      if (shareRegenTimeoutRef.current) {
        try { if (typeof window !== 'undefined' && window.cancelIdleCallback) window.cancelIdleCallback(shareRegenTimeoutRef.current); } catch (e) {}
        try { clearTimeout(shareRegenTimeoutRef.current); } catch (e) {}
        shareRegenTimeoutRef.current = null;
      }
    };
  ...
  const createShareCardBlob = async (details, config = shareConfig) => {
    try {
      ...
      // Cap DPR to avoid creating extremely large GPU textures on high-DPI displays
      const rawDpr = (typeof window !== 'undefined' && window.devicePixelRatio) ? window.devicePixelRatio : 1;
      const dpr = Math.max(1, Math.min(1.5, rawDpr));

      // Prefer OffscreenCanvas where available (can be used from workers and avoids main-thread compositing)
      const useOffscreen = typeof OffscreenCanvas !== 'undefined';
      let canvas;
      if (useOffscreen) {
        canvas = new OffscreenCanvas(Math.floor(width * dpr), Math.floor(height * dpr));
      } else {
      ...
      // Scale drawing operations to account for DPR so we can draw in CSS pixels
      ...
      // Convert to blob. OffscreenCanvas supports convertToBlob in many browsers
      ...
      if (typeof canvas.toBlob === 'function') {
        // toBlob can be slow; protect with a timeout to avoid hanging
        return await new Promise((resolve) => {
          let called = false;
          try {
            canvas.toBlob((b) => { called = true; resolve(b); }, 'image/png', 0.92);
          } catch (e) {
            called = true;
            resolve(null);
          }
          setTimeout(() => { if (!called) resolve(null); }, 5000);
        });
      }
  ...
  // 🚀 CONSOLIDATED: Modal and overlay states to reduce re-renders
  ...
  const updateModal = useCallback((modalKey, value) => {
    setModals(prev => {
      if (prev[modalKey] === value) return prev;
      return { ...prev, [modalKey]: value };
    });
  }, []);
  
  // Backward compatibility aliases
  const showStreamingPlayer = modals.showStreamingPlayer;
  const showEpisodeSelector = modals.showEpisodeSelector;
  const showCastDetails = modals.showCastDetails;
  const setShowStreamingPlayer = (v) => updateModal('showStreamingPlayer', v);
  const setShowEpisodeSelector = (v) => updateModal('showEpisodeSelector', v);
  const setShowCastDetails = (v) => updateModal('showCastDetails', v);
  ...
  // 🚀 ULTRA-FAST TRAILER: Enhanced trailer handlers with instant modal display
  const handleTrailerClick = useCallback(() => {
    // 🚀 CRITICAL OPTIMIZATION: Show modal IMMEDIATELY without waiting for player
    setShowTrailer(true);
    
    // Reduce initial loading state delay to feel more responsive
    setIsTrailerLoading(!youtubePlayerReady);

    // Preload YouTube player component immediately when trailer is clicked
    preloadYouTubePlayer().then(() => {
      if (isMountedRef.current) {
        setYoutubePlayerReady(true);
        // Clear loading state when YouTube is ready (if not already cleared by component mount)
        setTimeout(() => {
          if (isMountedRef.current && youtubePlayerReady) {
            setIsTrailerLoading(false);
          }
        }, 50);
      }
    }).catch(() => {
      // Silently fail but allow player to load anyway
      if (isMountedRef.current) {
        setYoutubePlayerReady(true);
      }
    });
    ...
    // Accessibility: Move focus to trailer player after modal is visible
    setTimeout(() => {
      if (!isMountedRef.current) return;
      if (playerRef.current && typeof playerRef.current.focus === "function") {
        playerRef.current.focus();
      }
    }, 200); // Reduced from 350ms for faster focus switch
  ...
  }, [movie, playerRef, movieDetails, youtubePlayerReady]);
  ...
  const handleStreamingClick = useCallback(() => {
    ...
    // Normalize poster_path and backdrop_path to TMDB paths
    const normalizeTmdbPath = (url) => {
      if (!url) return null;
    ...
    const movieForProgress = {
      ...movie,
      poster_path: normalizeTmdbPath(movie.poster_path || movie.poster),
      backdrop_path: normalizeTmdbPath(movie.backdrop_path || movie.backdrop)
    };

    // Track that user started watching this movie
    if ((movie.media_type || movie.type || 'movie') === 'movie') {
      startWatchingMovie(movieForProgress);
    }
  ...
  const [basicLoading, setBasicLoading] = useState(movie?._skipInitialLoad ? false : true);
  ...
  const fetchBasicInfo = useCallback(async () => {
    ...
    let fetchStart;
    try {
      // Determine if caller provided prefilled/basic fields to avoid showing the full-page loader
      const hasPrefill = !!(
        movie.poster_path || movie.poster || movie.backdrop || movie.backdrop_path || movie.overview || movie.title || movie.name
      );

      // If we already have prefilled data (or movieDetails for the same id), skip the basic loading skeleton
      const isSameAsCurrent = movieDetails && movieDetails.id === movie.id;
      const shouldShowBasicLoading = !(hasPrefill || isSameAsCurrent);

      if (isMountedRef.current && latestRequestRef.current === requestId) {
        if (shouldShowBasicLoading) {
      ...
      >
        ...
        >
            ...
            <div ref={scrollContainerRef} className="h-full overflow-y-auto hide-scrollbar">
              ...
              <div className="relative p-4 sm:p-8 bg-gradient-to-br from-black/60 via-black/30 to-primary/10 shadow-lg overflow-hidden transition-all duration-300">
                ...
                >
                  ...
                  {movie && (
                    <Suspense fallback={<div className="py-8 flex justify-center"><CardLoader /></div>}>
                      <EnhancedSimilarContent
                        ...
                        preloadOnMount={isHighPerformanceDevice()}
                        enableAggressivePreloading={false}
          ...
          <motion.button
            ...
            className={`absolute top-4 right-4 z-10 p-2 rounded-full text-white group overflow-hidden bg-[rgb(255,255,255,0.03)] ${supportsBackdropFilter() && isHighPerformanceDevice() ? 'backdrop-blur-[0.5px]' : ''} border-l-[1px] border-r-[1px] border-white/30 hover:bg-white/10 ${isHighPerformanceDevice() ? 'transform-gpu will-change-transform' : ''}`}
            variants={buttonVariants}
            initial="initial"
            whileHover="hover"
            whileTap="tap"
                        ...
                        <Suspense fallback={
                          ...
                          <LazyYouTube
                            ...
                            opts={{
                              ...
                              height: '100%',
                              playerVars: {
                                // 🚀 ULTRA-FAST: Optimized YouTube options for instant loading
                                autoplay: 0,
                                controls: 1,
                                modestbranding: 1,
                                ...
                                playsinline: 1,
                                // 🚀 NEW: Force faster loading
                                fs: 1, // Allow fullscreen
                                start: 0,
                                end: 0,
                                // Progressive download for faster playback
                                preload: 'auto', // Changed from 'metadata' to 'auto' for faster initial buffering
                              },
                            }}
                            onReady={(event) => {
        ...
        {/* Streaming Player (lazy, only mount when opened) */}
        {showStreamingPlayer && (
          <Suspense fallback={<div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50"><Loader size="large" color="white" variant="circular" /></div>}>
            <StreamingPlayer
