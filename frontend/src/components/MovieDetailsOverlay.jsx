...
// 🆕 NEW: Memory cleanup functions that were referenced but not defined
const emergencyMemoryCleanup = () => {
  console.warn('[MovieDetailsOverlay] Performing emergency memory cleanup');
  clearCache();
  ...
  if (typeof window !== 'undefined' && window.gc) {
    try {
      ...
      console.warn('[MovieDetailsOverlay] Manual GC failed:', e);
    }
  }
};

const aggressiveMemoryCleanup = () => {
  console.warn('[MovieDetailsOverlay] Performing aggressive memory cleanup');
  clearCache();
...
const preloadTrailerData = async (movieId, movieType) => {
  ...
  const useIsMobile = () => {
    ...
    // Desktop: Add performance throttling to prevent hanging
    const [isPerformanceThrottled, setIsPerformanceThrottled] = React.useState(() => {
      if (typeof window === 'undefined') return false;
      return window.innerWidth > 1024;
    });
...
const MovieDetailsOverlay = ({ movie, onClose, onMovieSelect, onGenreClick }) => {
  ...
  // 🚀 FIXED: Simplified cleanup on unmount - remove complex cleanup that causes hanging
  useEffect(() => {
    return () => {
      // Clear timeouts
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(optimizeImages, 200); // Faster execution
    return () => {
      if (timeoutId) {
        clearTimeout(timeoutId);
      }
  ...
  // 🚀 FIXED: Auto-regenerate preview without infinite loops
  useEffect(() => {
  ...
  }, [shareConfig, showShareSheet, sharePanelExpanded, movieDetails]); // Added movieDetails dependency for proper updates

  // 🚀 FIXED: handleToggleShareEdit with memory leak prevention
    ...
    try {
      ...
      if ((config.showBranding ?? true)) {
        ...
        try {
          ...
          ctx.globalAlpha = 1;
        } catch (e) { /* ignore icon load errors */ }
  ...
  }, [movie?.id, similarMoviesPage]); // Simplified dependencies to prevent infinite loops
  ...
  // 🚀 FIXED: Simplified fetchMovieData with desktop-specific optimizations
  ...
  // 🆕 NEW: Performance-optimized error boundary with adaptive retry
  const handleOptimizedRetry = useCallback(() => {
    ...
    handleRetry();
  }, [movie?.id, movie?.media_type, movie?.type, handleRetry]);
  ...
  const fetchSeasonEpisodes = useCallback(async (seasonNumber) => {
    ...
    // Prevent duplicate requests for the same season
  ...
  }, [movie, isEpisodesLoading]);

  // 🚀 FIXED: TV content loading without infinite loops
  useEffect(() => {
    if ((movie?.media_type === 'tv' || movie?.type === 'tv') && movie?.id) {
      ...
      const loadTVContent = async () => {
        try {
          ...
          if (seasonsData && seasonsData.length > 0) {
            ...
            // Only set current season if user hasn't manually selected one
            // This prevents overriding manual season selection
            if (!currentSeason) {
              const latestSeason = seasonsData[seasonsData.length - 1];
              setCurrentSeason(latestSeason);
              
              // Load episodes for the latest season only on initial load
              if (import.meta.env.DEV) {
                console.log('[Episode Loading] Initial load - episodes for season:', latestSeason.season_number);
              }
              // Call fetchSeasonEpisodes directly to avoid dependency issues
              if (movie?.id) {
                const seasonNumber = latestSeason.season_number;
                // Inline the episode fetching logic to avoid function dependencies
                if (!isEpisodesLoading) {
                  setIsEpisodesLoading(true);
                  try {
                    const seasonData = await getTVSeason(movie.id, seasonNumber);
            ...
            // User has manually selected a season - don't override their choice
            // Episodes will be loaded via handleSeasonChange when they change seasons
          }
        } catch (err) {
          console.error('Failed to load TV content independently:', err);
        }
      };
      
      loadTVContent();
    }
  }, [movie?.id, movie?.media_type, movie?.type, isEpisodesLoading]); // Removed fetchSeasonEpisodes dependency
  ...
  const handleSeasonChange = useCallback((season) => {
    ...
    // Inline episode fetching to avoid dependency on fetchSeasonEpisodes function
    if (movie?.id && season?.season_number) {
      const seasonNumber = season.season_number;
      if (!isEpisodesLoading) {
        setIsEpisodesLoading(true);
        getTVSeason(movie.id, seasonNumber)
          .then(seasonData => {
            setEpisodes(seasonData.episodes || []);
          })
          .catch(err => {
  ...
  }, [movie?.id, isEpisodesLoading]); // Removed fetchSeasonEpisodes dependency
  ...
  // 🚀 FIXED: Background refresh without infinite loops
  useEffect(() => {
    ...
    backgroundRefreshTimer = setTimeout(() => {
      ...
      if (latestRequestRef.current) {
        // Only refresh if we have a valid request ID
        const currentRequestId = latestRequestRef.current;
        // Call fetchMovieData directly without dependency
        fetchMovieData(0);
      }
  ...
  }, [movie?.id, movie?.media_type, movie?.type, movieDetails]); // Removed fetchMovieData dependency
  ...
  // 🚀 FIXED: Portal creation with memory leak prevention
  useEffect(() => {
    ...
    // 🚀 FIXED: Enhanced cleanup: Remove portal with memory leak prevention
    return () => {
      ...
      // Enhanced portal cleanup with memory leak prevention
      if (container && container.parentNode && isMountedRef.current) {
        try {
