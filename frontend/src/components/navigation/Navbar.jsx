...
const Navbar = ({ onMovieSelect, onCastSelect }) => {
  ...
  const handleSearch = useCallback(async (query) => {
    ...
    if (cachedResults && Date.now() - cachedResults.timestamp < SEARCH_CACHE_DURATION) {
    ...
    try {
      ...
      const results = await searchCombined(trimmedQuery, 1, { 
        ...
        includeAdult: true, // Better for general search
        language: 'en-US'
      });
      ...
      // Remove duplicates by id and media type for better deduplication
      ...
      if (isActive) {
        // Enhanced validation: ensure results are relevant to the current query
        const validatedResults = processedResults.filter(result => {
          const title = (result.title || result.name || '').toLowerCase();
          const originalTitle = (result.original_title || result.original_name || '').toLowerCase();
          const overview = (result.overview || '').toLowerCase();
          const query = trimmedQuery.toLowerCase();
          
          // Multiple validation criteria for better accuracy
          const hasExactMatch = title.includes(query) || originalTitle.includes(query);
          const hasFuzzyMatch = calculateStringSimilarity(query, title) > 0.3 || 
                               calculateStringSimilarity(query, originalTitle) > 0.3;
          const hasOverviewMatch = overview.includes(query);
          const hasWordMatch = query.split(/\s+/).some(word => 
            title.includes(word) || originalTitle.includes(word)
          );
          
          // Show result if any validation criteria is met
          return hasExactMatch || hasFuzzyMatch || hasOverviewMatch || hasWordMatch;
        });
  ...
  // Enhanced debounced search with suggestions and better performance
  const debouncedSearch = useMemo(() => {
    const debouncedFn = debounce((query) => handleSearch(query), 200); // Balanced delay for accuracy and speed
  ...
  const handleSearchSuggestions = useCallback(async (query) => {
    ...
    if (cachedSuggestions && Date.now() - cachedSuggestions.timestamp < SUGGESTION_CACHE_DURATION) {
    ...
    try {
      ...
      const historySuggestions = searchHistory
        .map(item => ({
          text: item,
          score: calculateStringSimilarity(trimmedQuery, item)
        }))
      ...
      // Add some trending suggestions (you can expand this with actual trending data)
      ...
      suggestionCache.set(suggestionKey, {
        data: allSuggestions,
        timestamp: Date.now()
      });
  ...
  const handleSearchChange = useCallback(async (e) => {
    ...
    if (query.trim()) {
      ...
      // Generate search predictions for better UX
      ...
      // Debounce the actual search for longer queries
      searchTimeoutRef.current = setTimeout(() => {
        debouncedSearch.search(query);
      }, query.trim().length < 4 ? 150 : 100); // Balanced response times for accuracy
    } else {
      debouncedSearch.cancel();
      setSearchResults([]);
      setIsSearching(false);
                              ...
                              <div className="flex items-start justify-between gap-4">
                                ...
                                <div className="flex items-center gap-1.5 opacity-0 group-hover/item:opacity-100 transition-all duration-300 transform translate-x-2 group-hover/item:translate-x-0">
                                  ...
                                  <motion.div
                                    ...
                                    className={`relative w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer group/btn
                                      ${movie.type === 'person' 
                                  ...
                                  <motion.div
                                    ...
                                    className="relative w-8 h-8 rounded-full bg-white/10 hover:bg-white/15 flex items-center justify-center transition-all duration-200 text-white/70 hover:text-white cursor-pointer group/share"
                                    title="Share"
                                    whileHover={{ scale: 1.1 }}
                                    whileTap={{ scale: 0.95 }}
                                  >
