      ...
      <div className="aspect-[2/3] rounded-xl overflow-hidden bg-gradient-to-br from-gray-800 to-gray-900 relative shadow-lg border border-white/5 hover:border-white/10 transition-all duration-300">
        {item.poster_path && typeof item.poster_path === 'string' && item.poster_path.startsWith('/') ? (
          <img 
            ...
            ref={imageRef}
            src={(() => {
              const imageUrl = correctSrcRef.current || getTmdbImageUrl(item.poster_path, 'w500');
              // Skip loading if this URL has already failed
              if (memoryOptimizer.failedImageUrls.has(imageUrl)) {
                if (import.meta.env.DEV) {
                  console.log(`Skipping previously failed image: ${imageUrl}`);
                }
                return ''; // Return empty string to trigger fallback immediately
              }
              return imageUrl;
            })()} 
            alt={displayTitle} 
            ...
            onLoadStart={() => {
              ...
              if (import.meta.env.DEV) {
                console.log('Similar content image loading started:', {
                  src: correctSrcRef.current || getTmdbImageUrl(item.poster_path, 'w500'),
                  poster_path: item.poster_path,
                  title: displayTitle
                });
              }
...
const EnhancedSimilarContent = React.memo(({ 
  ...
  // FIXED: Register cleanup with centralized memory manager - MEMORY LEAK FIX
  useEffect(() => {
    ...
    return () => {
      ...
      // NEW: Clear failed image URLs cache on unmount
      memoryOptimizer.failedImageUrls.clear();
  ...
  // Incremental enhancement: Pause work when tab is hidden to save resources
  useEffect(() => {
    ...
    >
      ...
      >
        ...
        <AnimatePresence onExitComplete={() => {
          ...
          // No intersection observer to clean up in this component
        }}>
