...
// Lightweight Animated wrapper to avoid heavy framer-motion work when animations are disabled
const Animated = ({ children, motionProps = {}, className = '', style = {}, as: As = 'div', ...rest }) => {
  const enabled = performanceOptimizer.getAnimationSettings().enableComplexAnimations && !(typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
  if (enabled) {
    return (
      <motion.div className={className} style={style} {...motionProps} {...rest}>
        {children}
      </motion.div>
    );
  }
  // Fallback plain div to reduce CPU/GPU usage
  const Component = As;
  return (
  ...
  return (
    <motion.div 
      ...
      transition={cardVariants ? { 
        ...
        stiffness: 220, 
        damping: 24, 
        duration: Math.max(0.12, performanceOptimizer.getAnimationSettings().duration) 
      } : undefined}
    >
      <div className="aspect-[2/3] rounded-xl overflow-hidden bg-gradient-to-br from-gray-800 to-gray-900 relative shadow-lg border border-white/5 hover:border-white/10 transition-all duration-300">
        {item.poster_path && typeof item.poster_path === 'string' && item.poster_path.startsWith('/') ? (
          <img 
            ...
            alt={displayTitle} 
            className="w-full h-full object-cover transition-transform duration-200 group-hover:scale-104" 
            loading="lazy"
            decoding="async"
            fetchpriority="low"
        ...
        {isOpening ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <motion.div 
              ...
              whileHover={performanceOptimizer.getAnimationSettings().enableHoverEffects ? { scale: 1.04 } : undefined}
              transition={{ duration: Math.min(0.18, performanceOptimizer.getAnimationSettings().duration) }}
            >
...
const EnhancedSimilarContent = React.memo(({ 
  ...
  const [displayedItems, setDisplayedItems] = useState(8); // Reduced initial items to 8 for lower memory/GPU usage
  ...
  const displayedContent = useMemo(() => {
    ...
    const HARD_CAP = 12; // never show more than 12 items at once
    const maxDisplayItems = Math.min(displayedItems, HARD_CAP);
  ...
  setDisplayedItems(8); // Align with initial displayed count to avoid jumps
  ...
  useEffect(() => {
    ...
    setSimilarContent([]);
  setDisplayedItems(8); // Reduced to 8 for lower memory/GPU usage
    setCurrentPage(1);
    setHasMore(true);
    setError(null);
    ...
    >
      ...
      >
        {displayedContent.map((item, index) => (
          <Animated
            key={item.id}
            motionProps={prefersReducedMotion ? {} : { initial: 'initial', animate: 'animate', variants: itemVariants, transition: { duration: 0.12 } }}
            className="group cursor-pointer relative"
...
const useSimpleIntersectionObserver = (callback, options = {}) => {
  ...
  const intersectionRafRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !hasIntersected) {
          ...
          // Use centralized scheduler for visibility-aware, throttled execution
          if (callbackRef.current) {
            // cancel any prior scheduled RAF
            if (intersectionRafRef.current) {
              try { cancelRaf(intersectionRafRef.current); } catch (_) {}
              intersectionRafRef.current = null;
            }
            intersectionRafRef.current = scheduleRaf(() => callbackRef.current());
            if (!intersectionRafRef.current) {
              // Fallback
              setTimeout(() => callbackRef.current(), 50);
            }
          }
        }
      },
    ...
    return () => {
      try {
        if (intersectionRafRef.current) {
          cancelRaf(intersectionRafRef.current);
          intersectionRafRef.current = null;
        }
      } catch (_) {}
      observer.disconnect();
