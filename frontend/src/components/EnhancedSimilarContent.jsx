...
const EnhancedSimilarContent = React.memo(({ 
  ...
  const fetchSimilarContent = useCallback(async (page = 1, append = false) => {
    ...
    try {
      ...
      // FIXED: Check if component is still mounted and request wasn't cancelled
      if (!isMountedRef.current) {
        return;
      }
      ...
      // FIXED: Check if component is still mounted before setting error state
      if (!isMountedRef.current) {
        return;
      }
  ...
  // FIXED: Handle load more without infinite loops
  ...
  }, [hasMore, loading, loadingMore, currentPage, contentId, contentType, maxItems]); // Removed fetchSimilarContent dependency
  ...
  // 🚀 PERFORMANCE OPTIMIZED: Fetch content on mount and when contentId/contentType changes with early preloading support
  useEffect(() => {
  ...
  }, [contentId, contentType]); // FIXED: Removed fetchSimilarContent dependency to prevent infinite loops
  ...
  // 🚀 ULTRA-SMOOTH: Performance-based cleanup and memory management with enhanced global cache management
  useEffect(() => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        if (performanceOptimizer.frameRate > 0) {
          performanceOptimizer.frameRate = 0;
        }
      } else {
        // Resume performance monitoring when visible
        if (performanceOptimizer.frameRate === 0) {
          performanceOptimizer.frameRate = 60;
        }
      }
    };
