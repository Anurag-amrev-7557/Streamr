 ...
 * NOTE: Disabled reduced motion check to enable animations on all devices
 */
export const isLowPowerDevice = () => {
  // Removed reduced motion check to always enable animations
  ...
  return isLowMemory || isSlowConnection; // Removed prefersReducedMotion
};

/**
 * Format animation properties based on device capabilities
 * NOTE: Always returns full animations to enable on mobile
 */
export const getAnimationConfig = () => {
  const isLowPower = isLowPowerDevice();
  
  // Always return full animations regardless of device
  return {
    duration: 0.2,
    transition: { duration: 0.2, ease: 'easeOut' },
    shouldAnimate: true,
    // Always use particles for better experience
    particleCount: 3,
  };
};
