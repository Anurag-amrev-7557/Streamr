...
export const setupGlobalErrorHandling = () => {
  ...
  console.error = (...args) => {
    try {
      ...
      // If there's an error in the error handler itself, fall back to original
      originalConsoleError.apply(console, args);
    }
  };
};
...
const isExternalResourceError = (error) => {
  ...
  // Safely convert error to string, handling circular references and non-primitive values
  let errorMessage = '';
  try {
    errorMessage = error.toString().toLowerCase();
  } catch (err) {
    // If toString() fails, try to get message or name
    try {
