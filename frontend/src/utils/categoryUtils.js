 ...
 * - Updates the query string for the current page to reflect category/genre/year.
 * - Avoids directly reading/writing window.location so callers (React components)
 *   can pass in Router-friendly values (location.search, location.pathname, navigate callback).
 *
 * Parameters:
 * - category: string (required)
 * - selectedGenre: object or string (optional)
 * - selectedYear: number|string (optional)
 * - options: {
 *     pathname?: string, // defaults to window.location.pathname when available
 *     search?: string,   // defaults to window.location.search when available
 *     replace?: (queryString: string) => void // optional callback to apply the new query (e.g. navigate)
 *   }
 */
export const updateURLWithCategory = (
  category,
  selectedGenre = null,
  selectedYear = null,
  options = {}
  ...
  try {
    const { pathname = (typeof window !== 'undefined' ? window.location.pathname : '/'), search = (typeof window !== 'undefined' ? window.location.search : ''), replace = null } = options;
    ...
    if (selectedGenre) {
      // selectedGenre may be an object with a name or a string
      const genreValue = typeof selectedGenre === 'object' ? (selectedGenre.name || selectedGenre.id || '') : selectedGenre;
      if (genreValue) searchParams.set('genre', String(genreValue).toLowerCase());
    }
    if (selectedYear) {
    ...
    if (typeof replace === 'function') {
      // Let the caller (e.g. React Router's navigate) handle history
      replace(query, { replace: true });
    } else if (typeof window !== 'undefined' && window.history && typeof window.history.replaceState === 'function') {
