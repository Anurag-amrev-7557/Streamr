  ...
  createPortal(id, options = {}) {
    ...
    if (!container) {
      ...
      Object.assign(container.style, {
        ...
        isolation: 'isolate'
        // NOTE: do not set will-change persistently as it forces layers and can increase GPU usage.
        // willChange is managed dynamically when animations are queued.
      });
    ...
    // Ensure will-change is only applied when animations are active for this portal
    if (this.animationService && typeof this.animationService.queueAnimation === 'function') {
      // Hook into animation queueing to apply will-change dynamically
      const originalQueue = this.animationService.queueAnimation.bind(this.animationService);
      this.animationService.queueAnimation = (portalId, anim) => {
        const p = this.portals.get(portalId);
        if (p && p.container && p.container.style) {
          p.container.style.willChange = 'transform, opacity';
        }
        const result = originalQueue(portalId, anim);
        // Remove will-change after a short timeout (animation should signal completion ideally)
        setTimeout(() => {
          const p2 = this.portals.get(portalId);
          if (p2 && p2.container && p2.container.style) {
            p2.container.style.willChange = '';
          }
        }, (anim && anim.duration) ? Math.min(anim.duration + 100, 1000) : 500);
        return result;
      };
    }
