// Get trending TV shows for today (like getTrendingMovies but only TV)
export const getTrendingTVShows = async (page = 1) => {
  ...
  try {
    const trendingData = await fetchWithRetry(
      `${getApiUrl()}/tmdb/trending?media_type=tv&time_window=day&page=${page}`
    );
    if (!trendingData?.results) {
      throw new Error('Invalid trending TV response');
    }
    // Only keep TV shows
    const uniqueTVMap = new Map();
    trendingData.results.forEach(item => {
      if (item.media_type === 'tv' && !uniqueTVMap.has(item.id)) {
        const transformed = transformTVData(item);
        if (transformed) {
          uniqueTVMap.set(item.id, transformed);
        }
      }
    });
    const tvShows = Array.from(uniqueTVMap.values());
    return {
      movies: tvShows,
...
// Get top rated movies released today
export const getTopRatedMoviesToday = async (page = 1) => {
  const MAX_RETRIES = 3;
  ...
  try {
    // Use discover API to get movies released today, sorted by vote_average
    const url = `${getApiUrl()}/tmdb/discover?media_type=movie&page=${page}&include_adult=false&language=en-US&region=US&primary_release_date.gte=${todayStr}&primary_release_date.lte=${todayStr}&sort_by=vote_average.desc&vote_count.gte=10`;
    ...
    console.error('Error fetching top rated movies for today:', error);
    return {
      movies: [],
      totalPages: 1,
      currentPage: page,
...
// Enhanced fetch availability check with robust polyfill and diagnostics
...
if (typeof fetch === 'function') {
  ...
  globalFetch = window.fetch.bind(window);
} else if (typeof global !== 'undefined' && typeof global.fetch === 'function') {
  globalFetch = global.fetch.bind(global);
} else if (typeof self !== 'undefined' && typeof self.fetch === 'function') {
  // For web workers
  globalFetch = self.fetch.bind(self);
} else {
  // No fetch found: provide a diagnostic error and a fallback
  const fetchErrorMsg = `
[tmdbService] Fetch API is not available in this environment.
Please ensure a fetch polyfill (e.g., 'whatwg-fetch' or 'cross-fetch') is loaded before using tmdbService.
Environment info:
  typeof window: ${typeof window}
  typeof global: ${typeof global}
  typeof self: ${typeof self}
  typeof fetch: ${typeof fetch}
  typeof window?.fetch: ${typeof window !== 'undefined' ? typeof window.fetch : 'N/A'}
  typeof global?.fetch: ${typeof global !== 'undefined' ? typeof global.fetch : 'N/A'}
  typeof self?.fetch: ${typeof self !== 'undefined' ? typeof self.fetch : 'N/A'}
`;

  // eslint-disable-next-line no-console
  console.error(fetchErrorMsg);

  // Fallback fetch that always throws
  globalFetch = () => {
    throw new Error('[tmdbService] Fetch is not available. Please load a fetch polyfill.');
  };
}
...
// Enhanced retry configuration - optimized for better reliability
const RETRY_CONFIG = {
  MAX_RETRIES: 2, // Increased to 2 for better reliability
  BASE_DELAY: 1000, // Base delay of 1 second
  MAX_DELAY: 3000, // Increased max delay to 3 seconds
  JITTER_FACTOR: 0.1, // Slight jitter for better distribution
...
const sharedFetchWithRetry = async (url, attempt = 1, customTimeout = RETRY_CONFIG.TIMEOUT) => {
  // On slow/very-slow networks, use enhancedNetworkService for adaptive timeouts,
  // concurrency limiting and in-flight de-duplication
  try {
    const profile = enhancedNetworkService.getNetworkProfile ? enhancedNetworkService.getNetworkProfile() : { bucket: 'fast' };
    if (profile.bucket !== 'fast') {
      const response = await enhancedNetworkService.performFetch(
        url,
        {
          headers: {
    ...
    // Fall through to native path on any issue obtaining profile/service
  }
  try {
...
// Enhanced cache with optimized durations for faster loading and size limits
const cache = new Map();
const CACHE_DURATIONS = {
  LIST: 10 * 60 * 1000,    // Reduced to 10 minutes for better memory management
  DETAILS: 30 * 60 * 1000, // Reduced to 30 minutes
  IMAGES: 6 * 60 * 60 * 1000 // Reduced to 6 hours for images
};
const MAX_CACHE_SIZE = 200; // Maximum number of cached items to prevent memory leaks

// Optimized request queue with better concurrency
const requestQueue = [];
let isProcessingQueue = false;
const MAX_CONCURRENT_REQUESTS = 1; // Reduced from 2 to 1 to prevent overwhelming the API
let activeRequests = 0;
let lastRequestTime = 0;
const MIN_REQUEST_INTERVAL = 500; // Increased from 200ms to 500ms to reduce API load
...
export const checkNetworkConnectivity = async () => {
  try {
    ...
    // Try to use the TMDB API directly as the primary connectivity test
    // This avoids CORS issues and tests the actual service we need
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 3000);
    
    try {
      const url = `${TMDB_BASE_URL}/configuration${TMDB_API_KEY ? `?api_key=${TMDB_API_KEY}` : ''}`;
      const response = await fetch(url, {
        method: 'HEAD',
        signal: controller.signal,
        cache: 'no-cache'
      ...
      clearTimeout(timeoutId);
      // Consider 401/403 as "connected" (auth failure ≠ offline)
      ...
      // If TMDB API fails, try a simple image request to a reliable CDN
      try {
        const imageController = new AbortController();
        const imageTimeoutId = setTimeout(() => imageController.abort(), 2000);
        
        // Use a reliable image CDN that supports CORS
        const imageResponse = await globalFetch('https://image.tmdb.org/t/p/w92/1E5baAaEse26fej7uHcjOgEE2t5.jpg', {
          method: 'HEAD',
        ...
        return imageResponse.ok;
                 ...
                 // Final fallback: check if we can make any network request
         try {
           // Try to fetch a simple text file from a reliable source
           const textController = new AbortController();
           const textTimeoutId = setTimeout(() => textController.abort(), 1500);
           
           const textResponse = await globalFetch('https://jsonplaceholder.typicode.com/posts/1', {
             method: 'GET',
           ...
           // If all else fails, return browser's online status
...
export const calculateRetryDelay = (attempt, error) => {
...
const setCachedData = (key, data, type = 'LIST') => {
  // Implement LRU eviction if cache is at maximum size
  if (cache.size >= MAX_CACHE_SIZE) {
    // Remove the oldest entry (simple FIFO eviction)
  ...
  cache.set(key, {
    ...
    timestamp: Date.now(),
    type,
    accessCount: 0, // For potential future LRU implementation
    lastAccessed: Date.now()
  });
};

// Cache cleanup function to remove expired entries and prevent memory leaks
const cleanupCache = () => {
  const now = Date.now();
  let cleanedCount = 0;
  
  for (const [key, value] of cache.entries()) {
    if (now - value.timestamp > CACHE_DURATIONS[value.type]) {
      cache.delete(key);
      cleanedCount++;
    }
  }
...
export const fetchWithCache = async (url, options = {}, type = 'LIST') => {
  ...
  const fetchWithRetry = async (url, attempt = 1) => {
    ...
    try {
      ...
      const response = await globalFetch(urlWithKey, {
        ...options,
        signal: controller.signal,
        headers: {
          'Accept': 'application/json',
...
export const getTrendingMovies = async (page = 1) => {
  ...
  const fetchWithRetry = async (url, attempt = 1) => {
    try {
      console.log(`🔄 Fetching trending movies (attempt ${attempt}): ${url}`);
      const response = await sharedFetchWithRetry(url, attempt);
      console.log(`✅ Trending movies response received:`, response.status);
      return await response.json();
    } catch (error) {
      console.error(`❌ Failed to fetch trending movies after ${attempt} attempts:`, {
        error: error.message,
        url: url,
        attempt: attempt
      });
  ...
  try {
    // Use backend proxy endpoint instead of calling TMDB directly
    const trendingData = await fetchWithRetry(
      `${getApiUrl()}/tmdb/trending?media_type=all&time_window=day&page=${page}`
    );
...
export const getPopularMovies = async (page = 1) => {
  ...
  try {
    const data = await fetchWithRetry(
      `${getApiUrl()}/tmdb/popular?media_type=movie&page=${page}&include_adult=false&language=en-US&region=US`
    );
...
export const getTopRatedMovies = async (page = 1) => {
  ...
  try {
    const data = await fetchWithRetry(
      `${getApiUrl()}/tmdb/top-rated?media_type=movie&page=${page}&include_adult=false&language=en-US&region=US`
    );
...
// 🚀 FIXED: Add cache to prevent duplicate API calls
const upcomingMoviesCache = new Map();
...
export const getUpcomingMovies = async (page = 1) => {
  ...
  const INITIAL_RETRY_DELAY = 1000;
  const MAX_RETRY_DELAY = 5000;
  const PAGES_TO_FETCH = 5; // 🚀 ENHANCED: Increased to 5 to ensure we get enough upcoming movies
  
  // 🚀 FIXED: Check cache first to prevent duplicate API calls
  const cacheKey = `upcoming_${page}`;
  const cached = upcomingMoviesCache.get(cacheKey);
  if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
    if (import.meta.env.DEV) {
      console.log(`🎬 Using cached upcoming movies for page ${page}`);
    }
    return cached.data;
  }
  ...
  try {
    // 🚀 FIXED: Use direct TMDB API for better upcoming movies data
    const fetchPromises = Array.from({ length: PAGES_TO_FETCH }, (_, i) => 
      fetchWithRetry(
        `${TMDB_BASE_URL}/movie/upcoming?api_key=${TMDB_API_KEY}&language=en-US&page=${page + i}&region=US&include_adult=false`
      ).catch(error => {
    ...
    if (import.meta.env.DEV) {
      console.log(`🎬 Raw upcoming movies data:`, {
        pageResults,
    ...
    if (import.meta.env.DEV) {
      console.log(`🎬 Upcoming movies filtering:`, {
        allMoviesCount: allMovies.length,
        upcomingMoviesCount: upcomingMovies.length,
        filteredOut: allMovies.length - upcomingMovies.length,
    ...
    // 🚀 ENHANCED: Better pagination logic for upcoming movies
    // Use a smaller page size to create more pages and better infinite scrolling
    const moviesPerPage = 20; // Fixed at 20 for consistent user experience
    const startIndex = (page - 1) * moviesPerPage;
    const endIndex = startIndex + moviesPerPage;
    const pageMovies = transformedMovies.slice(startIndex, endIndex);
    
    // 🚀 ENHANCED: More realistic total pages calculation
    // Calculate total pages based on available movies
    const totalPages = Math.max(page + 1, Math.ceil(transformedMovies.length / moviesPerPage));
    const hasMore = transformedMovies.length > endIndex; // Only false if we've reached the end of available movies
    
    if (import.meta.env.DEV) {
      console.log(`🎬 getUpcomingMovies pagination:`, {
        page,
        totalPages,
        hasMore,
        totalMovies: transformedMovies.length,
        pageMoviesCount: pageMovies.length,
        startIndex,
        endIndex,
        PAGES_TO_FETCH,
        availableMoviesAfterPage: transformedMovies.length - endIndex
      });
    }
      
    // 🚀 FIXED: Store in cache to prevent duplicate API calls
    const result = {
      movies: pageMovies, // 🚀 FIXED: Return only movies for the current page
      totalPages,
      currentPage: page,
      ...
      uniqueMoviesCount: transformedMovies.length,
      transformedMoviesCount: pageMovies.length, // 🚀 FIXED: Use page movies count
      // Enhanced error tracking
      errors: pageResults.filter(result => result.error).length
    };
    
    // Cache the result
    upcomingMoviesCache.set(cacheKey, {
      data: result,
      timestamp: Date.now()
    });
...
export const getActionMovies = async (page = 1) => {
  ...
  // 🚀 FIXED: Enhanced network error handling with connection reset protection
  const fetchWithRetry = async (url, attempt = 1) => {
    try {
      ...
      if (error.name === 'AbortError') {
        console.warn('Request timeout, aborting retry');
      ...
      if (error.message.includes('ERR_CONNECTION_RESET') || 
        ...
        console.warn('Network connection error detected:', error.message);
        
        // For network errors, use shorter delays and fewer retries
        if (attempt <= Math.min(MAX_RETRIES, 3)) {
          const delay = Math.min(1000 * Math.pow(2, attempt - 1), 4000);
          console.warn(`Network error, retrying in ${delay}ms (attempt ${attempt}/${Math.min(MAX_RETRIES, 3)})`);
          await sleep(delay);
          return fetchWithRetry(url, attempt + 1);
        }
      ...
      // Handle other errors with normal retry logic
      if (attempt <= MAX_RETRIES && error.name !== 'TypeError') {
        const delay = Math.min(INITIAL_RETRY_DELAY * Math.pow(2, attempt - 1), MAX_RETRY_DELAY);
        console.warn(`Request failed, retrying in ${delay}ms (attempt ${attempt}/${MAX_RETRIES}):`, error.message);
        await sleep(delay);
...
export const getMoviesByCategory = async (category, page = 1) => {
  ...
  const INITIAL_RETRY_DELAY = 1000;
  const MAX_RETRY_DELAY = 5000;
  const VALID_CATEGORIES = ['popular', 'top_rated', 'upcoming', 'now_playing', 'collections', 'netflix', 'prime', 'hbo', 'hulu', 'disney', 'apple'];
  ...
  try {
    ...
    // 🚀 FIXED: Special handling for upcoming movies with proper single-page fetching
    if (formattedCategory === 'upcoming') {
      // 🚀 ENHANCED: Use single-page fetching for proper pagination
      // This ensures we get exactly the page requested, not artificially sliced content
      const data = await fetchWithRetry(
      ...
      // Enhanced filtering for upcoming movies (only future release dates)
      const today = new Date();
      ...
      // 🚀 FIXED: Use TMDB's actual pagination instead of artificial slicing
      const totalPages = data.total_pages || 1;
      const hasMore = page < totalPages;

      if (import.meta.env.DEV) {
        console.log(`🎬 getMoviesByCategory upcoming pagination:`, {
          page,
          totalPages,
          hasMore,
          totalMovies: upcomingMovies.length,
          pageMoviesCount: upcomingMovies.length,
      ...
      return {
        movies: upcomingMovies, // 🚀 FIXED: Return all movies from the requested page
        totalPages,
        currentPage: page,
        totalResults: data.total_results || upcomingMovies.length,
        hasMore
      };
    }

    // 🚀 NEW: Special handling for streaming service categories
    if (['netflix', 'prime', 'hbo', 'hulu', 'disney', 'apple'].includes(formattedCategory)) {
      // Map category to network ID - Updated with correct TMDB watch provider IDs
      // These IDs are from TMDB's watch provider database
      const networkIdMap = {
        netflix: 8,      // Netflix
      ...
      const networkId = networkIdMap[formattedCategory];
      
      if (import.meta.env.DEV) {
        console.log(`🎬 getMoviesByCategory streaming service:`, {
          category: formattedCategory,
          networkId,
          page,
          validCategories: ['netflix', 'prime', 'hbo', 'hulu', 'disney', 'apple']
        });
      }

      // Use the getMoviesByNetwork function for streaming service categories
      try {
        const response = await getMoviesByNetwork(networkId, page);
        
        if (import.meta.env.DEV) {
          console.log(`🎬 getMoviesByCategory streaming service response:`, {
            category: formattedCategory,
            networkId,
            page,
            responseMovies: response.results?.length || 0,
        ...
        if (import.meta.env.DEV) {
          console.error(`🎬 Error in getMoviesByCategory for streaming service ${formattedCategory}:`, error);
        }
    ...
    // 🚀 NEW: Special handling for collections category
    if (formattedCategory === 'collections') {
      try {
        const response = await getPopularCollections(page);
        
        if (import.meta.env.DEV) {
          console.log(`🎬 getMoviesByCategory collections response:`, {
            category: formattedCategory,
            page,
            responseCollections: response.collections?.length || 0,
            responseTotalPages: response.totalPages,
            responseHasMore: response.hasMore
          });
        }
        ...
        if (import.meta.env.DEV) {
          console.error(`🎬 Error in getMoviesByCategory for collections:`, error);
        }
    ...
    // Standard API call for other categories
    const data = await fetchWithRetry(
      `${TMDB_BASE_URL}/movie/${formattedCategory}?api_key=${TMDB_API_KEY}&language=en-US&page=${page}&region=US&include_adult=false`
    );
...
export const getMoviesByNetwork = async (networkId, page = 1) => {
  const MAX_RETRIES = 3;
  const INITIAL_RETRY_DELAY = 1000;
  const MAX_RETRY_DELAY = 5000;
  ...
  const fetchWithRetry = async (url, attempt = 1) => {
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          const retryAfter = parseInt(response.headers.get('Retry-After')) || 60;
          console.warn(`Rate limit hit for movies by network ${networkId}, waiting ${retryAfter} seconds`);
          await sleep(retryAfter * 1000);
          return fetchWithRetry(url, attempt + 1);
        } else if (response.status >= 500 && attempt <= MAX_RETRIES) {
          const delay = Math.min(INITIAL_RETRY_DELAY * Math.pow(2, attempt - 1), MAX_RETRY_DELAY);
          console.warn(`Server error ${response.status} for movies by network ${networkId}, retrying in ${delay}ms (attempt ${attempt}/${MAX_RETRIES})`);
          await sleep(delay);
          return fetchWithRetry(url, attempt + 1);
        }
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      ...
      if (error.name === 'AbortError') {
        console.warn('Request timed out for movies by network');
      }
      
      if (attempt <= MAX_RETRIES && error.name !== 'TypeError') {
        const delay = Math.min(INITIAL_RETRY_DELAY * Math.pow(2, attempt - 1), MAX_RETRY_DELAY);
        console.warn(`Request failed for movies by network ${networkId}, retrying in ${delay}ms (attempt ${attempt}/${MAX_RETRIES}):`, error.message);
        await sleep(delay);
        return fetchWithRetry(url, attempt + 1);
      }
      throw error;
    ...
    try {
    // Enhanced movies by network fetching with better parameters and error handling
    // Use with_watch_providers for streaming service content
    const apiUrl = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_watch_providers=${networkId}&watch_region=US&language=en-US&page=${page}&sort_by=popularity.desc&include_adult=false&vote_count_gte=10`;
    
    if (import.meta.env.DEV) {
      console.log(`🎬 getMoviesByNetwork API call:`, {
        networkId,
        page,
        apiUrl: apiUrl.replace(TMDB_API_KEY, '***API_KEY***')
      });
    }
    ...
    if (!data || !data.results) {
      throw new Error('Invalid API response structure for movies by network');
    }

    if (import.meta.env.DEV) {
      console.log(`🎬 getMoviesByNetwork response:`, {
        networkId,
        page,
        totalResults: data.total_results,
        totalPages: data.total_pages,
        resultsCount: data.results?.length || 0,
        firstMovie: data.results?.[0]?.title || 'None'
      });
    }
    ...
    // Transform the results to match the expected format for MoviesPage
    ...
    // Calculate metadata
    const averageRating = transformedMovies.length > 0 ? 
      Math.round((transformedMovies.reduce((sum, movie) => sum + parseFloat(movie.rating || 0), 0) / transformedMovies.length) * 10) / 10 : 
      0;

    const averageVoteCount = transformedMovies.length > 0 ? 
      Math.round((transformedMovies.reduce((sum, movie) => sum + (movie.vote_count || 0), 0) / transformedMovies.length)) : 
      0;

    const releaseYearRange = transformedMovies.length > 0 ? {
      min: Math.min(...transformedMovies.map(movie => new Date(movie.release_date).getFullYear()).filter(y => y > 1900)),
      max: Math.max(...transformedMovies.map(movie => new Date(movie.release_date).getFullYear()).filter(y => y > 1900))
    } : null;
    ...
    return {
      ...
      hasMore: (data.page || page) < (data.total_pages || 1),
      category: 'network',
      categoryDisplayName: 'Network Movies',
      metadata: {
        averageRating,
        averageVoteCount,
        releaseYearRange,
    ...
    console.error(`Error fetching movies for network ${networkId}:`, error);
...
export const getTVSeason = async (tvId, seasonNumber) => {
  ...
  const fetchWithRetry = async (url, attempt = 1) => {
    try {
      ...
      if (attempt < 3) {
        console.warn(`[TMDB Service] Retry ${attempt} for TV season ${tvId}/${seasonNumber}:`, error.message);
        await sleep(1000 * attempt);
        return fetchWithRetry(url, attempt + 1);
      }
      throw error;
  ...
  try {
    console.log(`[TMDB Service] Fetching TV season ${tvId}/${seasonNumber}`);
    ...
    if (!data.episodes || !Array.isArray(data.episodes)) {
      console.warn(`[TMDB Service] No episodes array in response for season ${tvId}/${seasonNumber}:`, data);
    }
    ...
    console.log(`[TMDB Service] Successfully fetched TV season ${tvId}/${seasonNumber} with ${transformedSeason.episodes.length} episodes`);
    return transformedSeason;
  } catch (error) {
    console.error(`[TMDB Service] Error fetching TV season ${tvId}/${seasonNumber}:`, error);
    throw error;
  }
};
...
/**
 * Get detailed information about a person/actor
 * @param {number} personId - The TMDB person ID
 * @returns {Promise<Object>} Person details including biography, filmography, etc.
 */
export const getPersonDetails = async (personId) => {
  try {
    ...
    const url = `${TMDB_BASE_URL}/person/${personId}?api_key=${TMDB_API_KEY}&language=en-US&append_to_response=combined_credits,images,external_ids`;
    ...
    const transformedPerson = {
      ...
      profile_path: data.profile_path ? `${TMDB_IMAGE_BASE_URL}/w500${data.profile_path}` : null,
      ...
      combined_credits: data.combined_credits ? {
        cast: data.combined_credits.cast?.map(credit => ({
          id: credit.id,
          title: credit.title || credit.name,
          original_title: credit.original_title || credit.original_name,
          character: credit.character,
          media_type: credit.media_type,
          poster_path: credit.poster_path ? `${TMDB_IMAGE_BASE_URL}/w185${credit.poster_path}` : null,
          backdrop_path: credit.backdrop_path ? `${TMDB_IMAGE_BASE_URL}/w500${credit.backdrop_path}` : null,
        ...
        crew: data.combined_credits.crew?.map(credit => ({
          id: credit.id,
          title: credit.title || credit.name,
          original_title: credit.original_title || credit.original_name,
          department: credit.department,
      ...
      images: data.images ? {
        profiles: data.images.profiles?.map(profile => ({
          file_path: `${TMDB_IMAGE_BASE_URL}/w500${profile.file_path}`,
          width: profile.width,
 ...
 * @param {number} personId - The TMDB person ID
 * @returns {Promise<Object>} Combined credits with cast and crew roles
 */
export const getPersonCombinedCredits = async (personId) => {
  try {
    ...
    const url = `${TMDB_BASE_URL}/person/${personId}/combined_credits?api_key=${TMDB_API_KEY}&language=en-US`;
...
/**
 * Get person's images (profile photos)
 * @param {number} personId - The TMDB person ID
 * @returns {Promise<Object>} Person images including profile photos
 */
...
/**
 * Get person's external IDs (IMDB, social media, etc.)
 * @param {number} personId - The TMDB person ID
 * @returns {Promise<Object>} External IDs for the person
 */
 ...
 * @param {string} region - Region for search (default: 'US')
 * @returns {Promise<Object>} Search results for people
 */
export const searchPeople = async (query, page = 1, includeAdult = false, region = 'US') => {
  try {
    if (!query || query.trim().length === 0) {
      throw new Error('Search query is required');
    }

    const url = `${TMDB_BASE_URL}/search/person?api_key=${TMDB_API_KEY}&language=en-US&query=${encodeURIComponent(query.trim())}&page=${page}&include_adult=${includeAdult}&region=${region}`;
    ...
    return {
      ...
      results: data.results?.map(person => ({
        id: person.id,
        name: person.name,
        profile_path: person.profile_path ? `${TMDB_IMAGE_BASE_URL}/w185${person.profile_path}` : null,
        adult: person.adult,
        ...
        known_for: person.known_for?.map(item => ({
          ...
          poster_path: item.poster_path ? `${TMDB_IMAGE_BASE_URL}/w185${item.poster_path}` : null,
          backdrop_path: item.backdrop_path ? `${TMDB_IMAGE_BASE_URL}/w500${item.backdrop_path}` : null,
...
export const getPopularCollections = async (page = 1) => {
  const MAX_RETRIES = 3;
  const INITIAL_RETRY_DELAY = 1000;
  const MAX_RETRY_DELAY = 5000;
  ...
  const fetchWithRetry = async (url, attempt = 1) => {
    try {
      ...
      if (attempt <= MAX_RETRIES && error.message.includes("500")) {
        const delay = Math.min(INITIAL_RETRY_DELAY * Math.pow(2, attempt - 1), MAX_RETRY_DELAY);
        console.warn(`Retrying after ${delay}ms (attempt ${attempt}/${MAX_RETRIES})`);
        await sleep(delay);
        return fetchWithRetry(url, attempt + 1);
      }
      throw error;
  ...
  try {
    // Use search/collection endpoint with popular collection names as search terms
    const popularCollectionNames = [
      'Marvel', 'DC', 'James Bond', 'Star Wars', 'Harry Potter', 'Lord of the Rings',
      'Fast and Furious', 'Mission Impossible', 'Transformers', 'X-Men', 'Spider-Man',
      'Batman', 'Superman', 'Avengers', 'Pirates of the Caribbean', 'Toy Story',
      'Shrek', 'Despicable Me', 'Jurassic Park', 'Alien', 'Predator', 'Terminator',
      'Rocky', 'Rambo', 'Die Hard', 'Lethal Weapon', 'Indiana Jones', 'Back to the Future'
    ];
    
    // For now, let's search for some popular collection terms
    const searchTerms = popularCollectionNames.slice((page - 1) * 2, page * 2);
    let allCollections = [];
    
    for (const term of searchTerms) {
      try {
        const url = `${TMDB_BASE_URL}/search/collection?api_key=${TMDB_API_KEY}&language=en-US&query=${encodeURIComponent(term)}&page=1`;
        const data = await fetchWithRetry(url);
        
        if (data && data.results) {
          allCollections = allCollections.concat(data.results);
        }
      } catch (error) {
        console.warn(`Failed to search for collection term "${term}":`, error.message);
      }
    }
    ...
    const collections = uniqueCollections.map(collection => ({
      id: collection.id,
      ...
      parts_count: collection.parts?.length || 0
    }));


    return {
      collections,
      totalPages: Math.ceil(popularCollectionNames.length / 2), // Approximate pages
      currentPage: page,
      totalResults: collections.length,
      hasMore: page < Math.ceil(popularCollectionNames.length / 2)
    };
  } catch (error) {
...
export const getCollectionDetails = async (collectionId) => {
  ...
  try {
    const url = `${TMDB_BASE_URL}/collection/${collectionId}?api_key=${TMDB_API_KEY}&language=en-US`;
    const data = await fetchWithRetry(url);
    
    if (!data) {
      throw new Error("Invalid API response structure for collection details");
    }

    // Transform collection data with movies
    const collection = {
      id: data.id,
      name: data.name,
      overview: data.overview,
      ...
      parts: (data.parts || []).map(transformMovieData).filter(Boolean),
...
export const searchCollections = async (query, page = 1) => {
  ...
  try {
    const url = `${TMDB_BASE_URL}/search/collection?api_key=${TMDB_API_KEY}&language=en-US&query=${encodeURIComponent(query)}&page=${page}`;
    const data = await fetchWithRetry(url);
    
    if (!data || !data.results) {
      throw new Error("Invalid API response structure for collection search");
    }

    // Transform collection data
    const collections = data.results.map(collection => ({
      id: collection.id,
      name: collection.name,
      overview: collection.overview,
      poster_path: collection.poster_path,
    ...
    return {
      ...
      totalPages: data.total_pages || 1,
      currentPage: data.page || page,
      totalResults: data.total_results || collections.length,
      hasMore: (data.page || page) < (data.total_pages || 1)
    };
  } catch (error) {
