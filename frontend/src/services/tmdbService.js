...
// Ensure fetch is available (polyfill for Node.js environments)
...
if (typeof globalFetch === 'undefined') {
  if (typeof window !== 'undefined' && window.fetch) {
    ...
    // For frontend applications, use browser's fetch or provide a helpful error
    if (typeof window !== 'undefined') {
      // We're in a browser environment, fetch should be available
      globalFetch = window.fetch;
    } else {
      console.error('Fetch is not available in this environment. Please ensure fetch polyfill is loaded.');
      // Create a mock fetch that throws an error
      globalFetch = () => {
        throw new Error('Fetch is not available. Please load a fetch polyfill.');
      };
    }
  }
}
...
export const getMoviesByCategory = async (category, page = 1) => {
  ...
  const INITIAL_RETRY_DELAY = 1000;
  const MAX_RETRY_DELAY = 5000;
  const VALID_CATEGORIES = ['popular', 'top_rated', 'upcoming', 'now_playing', 'netflix', 'prime', 'hbo', 'hulu', 'disney', 'apple'];
