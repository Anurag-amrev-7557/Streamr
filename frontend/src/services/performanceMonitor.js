...
class PerformanceMonitor {
  constructor() {
    ...
    // FIXED: Only initialize in development with debug flag
  ...
  recordMetric(name, value) {
    ...
    if (name === 'memoryUsage' || name === 'memoryLimit') {
      // For memory metrics, only log if they're extremely high AND in debug mode (never in production)
      const memoryMB = value / 1024 / 1024;
      if (memoryMB > 1500 && import.meta.env.DEV && import.meta.env.VITE_DEBUG_MEMORY === 'true') {
  ...
  // Start monitoring - MEMORY LEAK FIX
  startMonitoring() {
    ...
    // Monitor memory usage - MEMORY LEAK FIX
    if ('memory' in performance && import.meta.env.DEV && import.meta.env.VITE_DEBUG_MEMORY === 'true') {
      this.memoryInterval = setInterval(() => {
        const memory = performance.memory;
        this.recordMetric('memoryUsage', memory.usedJSHeapSize);
        this.recordMetric('memoryLimit', memory.jsHeapSizeLimit);
      }, 300000); // FIXED: Every 5 minutes instead of 30 seconds
    }
