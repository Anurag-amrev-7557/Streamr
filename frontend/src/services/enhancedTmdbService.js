...
class EnhancedTmdbService {
  constructor() {
    this.cacheConfig = {
      movieDetails: 30 * 60 * 1000, // 30 minutes - reduced for fresher data
      movieList: 15 * 60 * 1000,    // 15 minutes - reduced for faster updates
      images: 24 * 60 * 60 * 1000,  // 24 hours - keep images cached longer
      search: 10 * 60 * 1000,       // 10 minutes - reduced for better search results
      genres: 24 * 60 * 60 * 1000   // 24 hours - genres don't change often
    };
    
    this.rateLimitConfig = {
      maxRequests: 50, // Increased for faster parallel loading
  ...
  async makeRequest(endpoint, params = {}, cacheKey = null, cacheTTL = null) {
    ...
    // Check cache first - prioritize cache hits for faster loading
    ...
    try {
      ...
      const timeoutId = setTimeout(() => controller.abort(), 8000); // Reduced from default to 8 seconds
