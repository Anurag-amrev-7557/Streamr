...
class EnhancedSimilarContentService {
  ...
  // Lightweight timeout wrapper for fetch-like promises
  withTimeout(promise, { ms = 1500, signal } = {}) {
    if (!ms) return promise;
    return new Promise((resolve) => {
      let settled = false;
      const timer = setTimeout(() => {
        if (!settled) resolve([]); // resolve empty on timeout for speed
      }, ms);
      promise.then((v) => {
        if (!settled) {
          settled = true;
  ...
  async getEnhancedSimilarContent(contentId, contentType = 'movie', options = {}) {
    const {
      ...
      fastStart = true // NEW: return shallow results quickly on first page
    } = options;
    ...
    try {
      ...
      // FAST PATH: shallow, low-latency fetch on initial page
      if (fastStart && page === 1 && !forceRefresh) {
        const shallowPromises = [
          this.withTimeout(this.fetchRecommendations(contentId, contentType, page, { signal }), { ms: 1200, signal }),
          this.withTimeout(this.fetchSimilar(contentId, contentType, page, { signal }), { ms: 1200, signal })
        ];
        const shallowResults = await Promise.all(shallowPromises);
        let combined = [];
        shallowResults.forEach(arr => { if (Array.isArray(arr)) combined.push(...arr); });
        combined = this.removeDuplicates(combined, { strategy: 'basic' });
        // Lightweight scoring without extra network calls
        const quick = combined.slice(0, Math.min(16, limit)).map(item => ({
          ...item,
          similarityScore: Math.min(1, Math.max(0, (item.vote_average ? item.vote_average / 10 : 0.2) * 0.7 + (item.popularity ? Math.min(item.popularity / 100, 0.3) : 0)))
        })).sort((a,b) => (b.similarityScore||0) - (a.similarityScore||0));

        // Fire-and-forget enrichment to warm cache
        (async () => {
          try {
            const full = await this.getEnhancedSimilarContent(contentId, contentType, { limit, minScore, includeDetails, forceRefresh, page, infiniteLoading, userId, useCulturalContext, fastStart: false });
            const warmKey = `enhanced_similar_${contentType}_${contentId}_${page}_${userId || 'global'}`;
            if (full && full.length) {
              this.cache.set(warmKey, full, { ttl: 30 * 60 * 1000, priority: 'high', namespace: 'similar_content' });
            }
          } catch (_) {}
        })();
  ...
  async getDetailedContent(contentList, contentType, { signal } = {}) {
    // Limit the number of detailed requests and use bounded concurrency for speed
    const limitedList = contentList.slice(0, 12);
    ...
    const mapper = async (item, index) => {
      try {
        ...
        // Small, jittered spacing to avoid bursty rate-limits
        if (index > 0) await new Promise(r => setTimeout(r, 60 + Math.floor(Math.random()*40)));
    ...
    // Run with bounded concurrency of 3
    const detailedResults = await this.mapWithConcurrency(limitedList, 3, mapper);
