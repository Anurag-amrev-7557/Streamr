  ...
  setupInterceptors() {
    ...
    this.errorInterceptor = (error) => {
      ...
      if (error.response?.status === 429) {
        ...
        console.warn(`Rate limited (429). Waiting ${retryAfter}s before retry ${error.config?.metadata?.retryCount + 1}/${this.retryConfig.maxRetries}`);
        
        // Add exponential backoff for rate limiting
        const delay = Math.min(
          retryAfter * 1000 * Math.pow(2, error.config?.metadata?.retryCount || 0),
          this.retryConfig.maxDelay
        );
        
        // Wait before retry
        setTimeout(() => {
          if (error.config?.metadata?.retryCount < this.retryConfig.maxRetries) {
            error.config.metadata.retryCount++;
            error.config.metadata.isRetry = true;
            this.makeRequest(error.config);
          }
        }, delay);
      ...
      if (error.config?.metadata?.retryCount < this.retryConfig.maxRetries) {
        const delay = Math.min(
          this.retryConfig.baseDelay * Math.pow(this.retryConfig.backoffMultiplier, error.config.metadata.retryCount),
          this.retryConfig.maxDelay
        );
        
        console.log(`Request failed (attempt ${error.config.metadata.retryCount + 1}/${this.retryConfig.maxRetries}): ${error.message}`);
