...
export const checkNetworkHealth = async () => {
  const checks = [
    ...
    },
    {
      name: 'Backend API',
      check: async () => {
        // Use centralized health check service if available
        try {
          const { healthCheckService } = await import('./healthCheckService.js');
          ...
          try {
            ...
            return { status: response.ok ? 'success' : 'failed', httpStatus: response.status };
          } catch (error) {
            clearTimeout(timeoutId);
            throw error;
          }
  ...
  for (const check of checks) {
    try {
      ...
      if (check.check) {
        ...
        clearTimeout(timeoutId);
        result = { status: 'success', responseTime: Date.now() };
      }
