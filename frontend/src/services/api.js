...
export const userAPI = {
  ...
  removeFromWatchlist: async (tmdbId, type) => {
    const { timeout } = getNetworkAwareConfig();
    const token = localStorage.getItem('accessToken');
    ...
    return fetchWithRetry(async () => {
      const response = await fetch(`${getApiUrl()}/user/watchlist/${tmdbId}/${type}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  syncViewingProgress: async (viewingProgress) => {
    const { timeout } = getNetworkAwareConfig();
    const token = localStorage.getItem('accessToken');
    ...
    return fetchWithRetry(async () => {
      const response = await fetch(`${getApiUrl()}/user/viewing-progress/sync`, {
        ...
        body: JSON.stringify({ viewingProgress })
      });
