...
export const useStreamingIconAnimation = (activeCategory, options = {}) => {
  ...
  useEffect(() => {
    ...
    let start;
    const duration = 500;
    // Throttle animation to reduce main-thread churn
    const targetFps = 30;
    ...
    function animate(ts) {
      ...
      if (progress < 1) {
        ...
        // Use shared scheduler which performs throttling and visibility checks
        animationFrameRef.current = scheduleRaf(animate);
      }
    }
    animationFrameRef.current = scheduleRaf(animate);
    ...
    return () => {
      if (transitionTimeoutRef.current) clearTimeout(transitionTimeoutRef.current);
      if (animationFrameRef.current) cancelRaf(animationFrameRef.current);
    };
  }, [activeCategory]);
