...
async function handleApiRequest(request) {
  ...
  try {
    ...
    // Cache successful JSON responses only, and skip Comick proxy
    const ct = networkResponse.headers.get('content-type') || '';
    const isJson = ct.toLowerCase().includes('application/json');
    const urlStr = request.url || '';
    const isComickProxy = urlStr.includes('/api/v1/comick') || urlStr.includes('/api/comick');
    if (networkResponse.ok && isJson && !isComickProxy) {
